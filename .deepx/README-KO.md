...
│   ├── dx-agent-app-build-python/     # Python 추론 앱 빌드
│   │   └── SKILL.md
│   ├── dx-agent-app-build-cpp/        # C++ 추론 앱 빌드
│   │   └── SKILL.md
│   ├── dx-agent-app-build-async/      # 비동기 고성능 앱 빌드
│   │   └── SKILL.md
│   ├── dx-agent-app-model-management/ # 모델 다운로드 및 레지스트리
│   │   └── SKILL.md
│   ├── dx-agent-app-validate/         # 5단계 검증 피라미드
│   │   └── SKILL.md
│   ├── dx-agent-brainstorm/           # 프로세스 skill — 코드 전 브레인스토밍
│   │   └── SKILL.md
│   ├── dx-agent-tdd/                  # 프로세스 skill — 테스트 주도 개발
│   │   └── SKILL.md
│   └── dx-agent-verify/               # 프로세스 skill — 완료 선언 전 검증; 필수 산출물 시행
│       └── SKILL.md
...
전체 가이드는 suite 루트 tooling 가이드를 참조하세요.
...
| Codex CLI | `AGENTS.md`, `.codex/skills/dx-codex-identity/SKILL.md` (자동), 그리고 `.deepx/skills/*/SKILL.md` 직접 참조 |
