...
>
> **Versions:** dx_rt 3.3.0 / dx-runtime v2.3.0
...
The `Configuration` singleton provides runtime settings and feature toggles via the
`ITEM` enum. As of dx_rt 3.3.0, the ITEM enum includes:

- `NFH_ASYNC = 11` — NFH async mode
- `NFH_ACCELERATION = 12` — NFH acceleration (conditional, compile-time)
- `CPU_OP_ACCELERATION = 13` — CPU operation acceleration (conditional, compile-time)

Additional methods beyond version queries:

| Method | Description |
|---|---|
| `set_fw_config_with_json(json_str)` | Configure firmware with a JSON string |
| `load_config_file(file_name)` | Load configuration from a file |
| `set_enable(item, enabled)` | Enable/disable a feature by ITEM enum |
| `get_enable(item)` | Check if a feature is enabled |
| `set_attribute(item, attrib, value)` | Set a feature attribute |
| `get_attribute(item, attrib)` | Get a feature attribute value |

See `dx_rt/python_package/src/dx_engine/configuration.py` for the complete ITEM enum,
ATTRIBUTE enum, and method list.
