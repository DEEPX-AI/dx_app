...
def check_ifactory_implementation(app_dir: Path) -> CheckResult:
    ...
    for factory_file in factory_files:
        try:
            ...
            # Find the first class that implements all required methods
                ...
                for node in all_classes:
                    ...
                    if missing:
                        missing_classes.append(
                            f"{node.name} (missing: {', '.join(sorted(missing))})"
                        )
                return CheckResult(
                    "ifactory", False,
                    f"{factory_file.name}: No class implements all 5 IFactory methods. "
                    f"Classes found: {'; '.join(missing_classes) if missing_classes else 'none'}"
                )
            
            # Check constructor accepts config
            init_methods = [
                n for n in factory_class.body
            ...
            if init_methods:
                ...
                if "config" not in arg_names:
                    return CheckResult(
                        "ifactory", False,
                        f"{factory_file.name}: {factory_class.name}.__init__ missing "
                        f"'config' parameter",
                        severity="warning"
                    )
        except SyntaxError:
...
def check_app_yaml(app_dir: Path) -> CheckResult:
    ...
    try:
        ...
        for key in required_keys:
            # Check only non-commented lines to avoid false positives from commented keys
            key_found = any(
...
def smoke_test_import(app_dir: Path) -> CheckResult:
    ...
    # Build import test script — use repr() to safely escape the path,
    # preventing code injection if app_dir contains quotes or special characters.
    test_code = f"""
import sys
from pathlib import Path
_module_dir = Path({repr(str(app_dir))})
