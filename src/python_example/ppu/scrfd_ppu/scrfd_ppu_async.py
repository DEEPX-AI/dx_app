...
class SCRFD_PPU:
    ...
    def convert_to_original_coordinates(self, detections: np.ndarray) -> np.ndarray:
        ...
        for i in range(self.num_keypoints):
            detections[:, 6 + i * 2] = np.clip(
                (detections[:, 6 + i * 2] - self.pad[1]) / self.gain,
                0,
                self.img_width - 1,
            )
            detections[:, 6 + i * 2 + 1] = np.clip(
                (detections[:, 6 + i * 2 + 1] - self.pad[0]) / self.gain,
                0,
                self.img_height - 1,
            )
    ...
    def draw_detections(self, img: np.ndarray, detections: np.ndarray) -> None:
        ...
        for detection in detections:
            ...
            for k in range(self.num_keypoints):
                kx = int(keypoints[k * 2])
                ky = int(keypoints[k * 2 + 1])
    ...
    def postprocess(self, output_tensors: List[np.ndarray]) -> np.ndarray:
        ...
        if output_tensor.shape[1] != 64:
            print(f"[ERROR] Expected 64 channels, got {output_tensor.shape[1]}")
            return np.empty((0, 16), dtype=np.float32)
        ...
        keypoints_raw = (
            output_tensor[:, 24:].view(np.float32).reshape(-1, self.num_keypoints, 2)
        )
        ...
        keypoints_decoded = np.zeros_like(keypoints_raw)
        keypoints_decoded[:, :, 0] = (keypoints_raw[:, :, 0] + gX[:, None]) * stride[
            :, None
        ]
        keypoints_decoded[:, :, 1] = (keypoints_raw[:, :, 1] + gY[:, None]) * stride[
            :, None
        ]
        keypoints_flat = keypoints_decoded.reshape(len(boxes), -1)
        ...
        if len(indices) > 0:
            ...
            class_ids = np.zeros(len(keep), dtype=np.float32)
            return np.column_stack(
                [boxes_x1y1x2y2[keep], scores[keep], class_ids, keypoints_flat[keep]]
            ).astype(np.float32)
