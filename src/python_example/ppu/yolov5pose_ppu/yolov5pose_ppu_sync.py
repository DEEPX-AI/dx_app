...
class YOLOv5Pose_PPU:
    ...
    def postprocess(self, output_tensors: List[np.ndarray]) -> np.ndarray:
        ...
        if output_tensor.shape[1] != 256:
            print(f"[ERROR] Expected 256 channels, got {output_tensor.shape[1]}")
            return np.empty((0, 57), dtype=np.float32)
        ...
        keypoints_raw = (
            output_tensor[:, 28:232].view(np.float32).reshape(-1, self.num_keypoints, 3)
        )
        ...
        keypoints_decoded[:, :, 0] = (
            keypoints_raw[:, :, 0] * 2.0 - 0.5 + gX[:, None]
        ) * stride[:, None]
        keypoints_decoded[:, :, 1] = (
            keypoints_raw[:, :, 1] * 2.0 - 0.5 + gY[:, None]
        ) * stride[:, None]
        keypoints_decoded[:, :, 2] = 1.0 / (1.0 + np.exp(-keypoints_raw[:, :, 2]))
