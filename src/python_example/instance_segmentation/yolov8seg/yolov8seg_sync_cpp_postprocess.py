...
class YOLOv8Seg:
    ...
    def image_inference(self, image_path: str, display: bool = True):
        ...
        detections, masks = self.postprocessor.postprocess(output_tensors)

        t3 = time.perf_counter()
        detections_scaled, masks_scaled = self.convert_to_original_coordinates(
            detections, masks
        )

        self.draw_detections(img, detections_scaled, masks_scaled)
