...
class AsyncRunner:
    ...
    def _run_visualize(self, frame: np.ndarray, results) -> np.ndarray:
        ...
        if hasattr(self.visualizer, "set_source_path"):
            self.visualizer.set_source_path(self._input_path or None)
    ...
    def _run_image(self, image_path: str, display: bool) -> None:
        ...
        if self._verbose:
            ...
            try:
                img_probe = load_display_frame(
                    image_path,
                    input_height=self.input_height,
                    input_width=self.input_width,
                ) if is_lidar_input(image_path) else cv2.imread(image_path)
            except Exception:
                img_probe = None
        ...
        for loop_idx in range(self._loop):
            ...
            # Create a single-frame iterable (LiDAR .bin → BEV frame; else image)
            input_source = iter([load_display_frame(
                image_path,
                input_height=self.input_height,
                input_width=self.input_width,
            )])
    ...
    def _run_image_dir(self, dir_path: str, display: bool) -> None:
        ...
        for loop_idx in range(self._loop):
            ...
            # Create iterator that loads frames (LiDAR .bin → BEV frame; else image)
