...
class YOLOv26Obb:
    ...
    def __init__(self, model_path: str):
        ...
        self.classes = get_labels("dotav1")
    ...
    def xywhr2xyxyxyxy(self, x: np.ndarray) -> np.ndarray:
    ...
    def convert_to_original_coordinates(self, detections: np.ndarray) -> np.ndarray:
        ...
        rboxes[:, 0] = (rboxes[:, 0] - self.pad[1]) / self.gain
        rboxes[:, 1] = (rboxes[:, 1] - self.pad[0]) / self.gain
        rboxes[:, 2] = rboxes[:, 2] / self.gain
        rboxes[:, 3] = rboxes[:, 3] / self.gain

        rboxes[:, 0] = np.clip(rboxes[:, 0], 0, self.img_width - 1)
        rboxes[:, 1] = np.clip(rboxes[:, 1], 0, self.img_height - 1)
        rboxes[:, 2] = np.clip(rboxes[:, 2], 0, self.img_width)
        rboxes[:, 3] = np.clip(rboxes[:, 3], 0, self.img_height)

        angles = detections[:, 6:7]
        rboxes_with_angle = np.concatenate([rboxes, angles], axis=-1)
        rboxes_reg = self.regularize_rboxes(rboxes_with_angle)

        detections[:, :4] = rboxes_reg[:, :4]
        detections[:, 6] = rboxes_reg[:, 4]
    ...
    def draw_detections(self, img: np.ndarray, detections: np.ndarray) -> None:
        ...
        for detection in detections:
            cx, cy, w, h, score, class_id, angle_val = detection

            color = self.color_palette[int(class_id)]

            rbox = np.array([[cx, cy, w, h, angle_val]], dtype=np.float32)
            poly = self.xywhr2xyxyxyxy(rbox)[0].astype(np.int32)
            ...
            # Place the label near the top-left corner of the rotated box
