...
class YOLOv7:
    ...
    def postprocess(self, output_tensors: List[np.ndarray]) -> np.ndarray:
        ...
        cls_scores = filtered_outputs[:, 5:]
        cls_max_scores = np.max(cls_scores, axis=1)
        cls_ids = np.argmax(cls_scores, axis=1)

        final_confidences = filtered_obj_scores * cls_max_scores
        ...
        if len(indices) > 0:
            ...
            return np.column_stack(
                [boxes_x1y1x2y2[keep], final_confidences[keep], cls_ids[keep]]
            ).astype(np.float32)
