...
class YOLOv5Face:
    ...
    def postprocess(self, output_tensors: List[np.ndarray]) -> np.ndarray:
        ...
        filtered_outputs = outputs[obj_mask]
        filtered_obj_scores = obj_scores[obj_mask]

        keypoints = filtered_outputs[:, 5:15]
        class_score = filtered_outputs[:, 15]

        final_confidences = filtered_obj_scores * class_score

        # (center_x, center_y, width, height)
        boxes_cxcywh = filtered_outputs[:, :4]
        ...
        if len(indices) > 0:
            ...
            class_ids = np.zeros(len(keep))  # All faces (class 0)
            return np.column_stack(
                [
                    boxes_x1y1x2y2[keep],
                    final_confidences[keep],
                    class_ids,
                    keypoints[keep],
                ]
            ).astype(np.float32)
