...
def parse_args():
    return parse_common_args("YOLOv5n Async Inference (C++ Postprocess)")
def main():
    args = parse_args()
    ...
    def on_engine_init(runner):
        ...
        # Mirror YOLOv5Postprocessor threshold mapping so the C++ path matches
        # the Python fallback exactly (config is post-alias: score_threshold
        # is also exposed as conf_threshold by the factory).
