...
class SCRFD:
    ...
    def postprocess(self, output_tensors: List[np.ndarray]) -> np.ndarray:
        ...
        triplets = [
            (
                self.input_width // max(1, int(round(np.sqrt(n // num_anchors)))),
                d["score"],
                d["bbox"],
                d["kps"],
            )
        ...
        keypoints_list = np.vstack(all_keypoints)
        ...
        if len(indices) > 0:
            ...
            filtered_boxes = boxes_x1y1wh[keep]
            filtered_scores = scores[keep]
            filtered_keypoints = keypoints_list[keep]
            ...
            boxes_x1y1x2y2 = np.column_stack(
                [
                    filtered_boxes[:, 0],
                    filtered_boxes[:, 1],
                    filtered_boxes[:, 0] + filtered_boxes[:, 2],
                    filtered_boxes[:, 1] + filtered_boxes[:, 3],
                ]
            )

            # All faces (class 0)
            class_ids = np.zeros(len(indices))
