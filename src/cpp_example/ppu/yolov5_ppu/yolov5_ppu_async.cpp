...
/**
 * @brief Asynchronous post-processing example for YOLOv5s PPU object detection
model.
 *
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv5PPUResult>>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv5s PPU Post-Processing Async Example";
    ...
    auto post_processor =
        YOLOv5PPUPostProcess(input_width, input_height, 0.25f, 0.3f, 0.45f);
