    ...
    while (appQuit->load() == 0) {
        if (wait_queue->size() > 0) {
            ...
            // detections 멤버는 포인터이므로, 벡터를 동적할당하여 복사
            {
                ...
                display_args->detections = new std::vector<YOLOv5PPUResult>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    if (!imgFile.empty()) {
        ...
        if (profiling_metrics.infer_completed > 0) {
            ...
            std::cout << "[PROFILE] ======== YOLOv5 Async Performance Report ========" << std::endl;
