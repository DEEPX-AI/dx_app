...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        try {
            detections_vec = args->spp->postprocess(outputs);
        ...
        d_args->detections = std::make_shared<std::vector<SCRFDPPUResult>>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor =
        SCRFDPPUPostProcess(input_width, input_height, 0.5f, 0.45f);
