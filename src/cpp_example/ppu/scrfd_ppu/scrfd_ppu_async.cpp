 ...
 */
void scale_coordinates(SCRFDPPUResult& detection, const std::vector<int>& pad_xy,
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, std::vector<SCRFDPPUResult>& detections,
    ...
    while (appQuit->load() == 0) {
        if (wait_queue->size() > 0) {
            ...
            // detections 멤버는 포인터이므로, 벡터를 동적할당하여 복사
            {
                ...
                display_args->detections = new std::vector<SCRFDPPUResult>(detections_vec);
                ...
                // spp: pointer to SCRFD_PPU_PostProcess instance
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = SCRFDPPUPostProcess(input_width, input_height, 0.6, 0.45);
