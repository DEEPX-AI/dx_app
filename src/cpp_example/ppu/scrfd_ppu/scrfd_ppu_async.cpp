...
/**
 * @brief Asynchronous post-processing example for SCRFD face detection model.
 *
...
struct DisplayArgs {
    std::shared_ptr<std::vector<SCRFDPPUResult>> detections;
    std::shared_ptr<cv::Mat> original_frame;

    SCRFDPPUPostProcess *spp = nullptr;
...
bool process_postprocess(SCRFDPPUPostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<SCRFDPPUResult>& detections_vec) {
    try {
...
/**
 * @brief Transform bounding box coordinates from letterbox padded/scaled image back
 * to original image coordinates.
 * @param box Bounding box [x1, y1, x2, y2] to transform (modified in place)
 * @param pad_xy [x, y] vector for padding size
 * @param ratio [x, y] vector for scale ratio
 * @param orig_width Original image width for clamping
 * @param orig_height Original image height for clamping
 */
void transform_box_to_original(std::vector<float>& box, const std::vector<int>& pad_xy,
    ...
    box[0] = (box[0] - static_cast<float>(pad_xy[0])) / ratio[0];
    box[1] = (box[1] - static_cast<float>(pad_xy[1])) / ratio[1];
    box[2] = (box[2] - static_cast<float>(pad_xy[0])) / ratio[0];
    box[3] = (box[3] - static_cast<float>(pad_xy[1])) / ratio[1];
...
/**
 * @brief Transform landmark coordinates from letterbox padded/scaled image back
 * to original image coordinates.
 * @param landmarks Landmarks vector [x1, y1, x2, y2, ...] to transform (modified in place)
 * @param pad_xy [x, y] vector for padding size
 * @param ratio [x, y] vector for scale ratio
 * @param orig_width Original image width for clamping
 * @param orig_height Original image height for clamping
 */
void transform_landmarks_to_original(std::vector<float>& landmarks, const std::vector<int>& pad_xy,
                                     const std::vector<float>& ratio, int orig_width, int orig_height) {
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, const std::vector<SCRFDPPUResult>& detections,
    ...
    for (const auto& detection : detections) {
        ...
        if (!landmarks.empty()) {
            transform_landmarks_to_original(landmarks, pad_xy, ratio, frame.cols, frame.rows);
        }
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        std::vector<SCRFDPPUResult> detections_vec;
        if (!process_postprocess(*args->spp, outputs, detections_vec)) {
            appQuit->store(1);
            continue;
        }
        ...
        d_args->detections = std::make_shared<std::vector<SCRFDPPUResult>>(std::move(detections_vec));
...
CommandLineArgs parse_command_line(int argc, char* argv[]) {
    ...
    std::string app_name = "SCRFD PPU Post-Processing Async Example";
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, SCRFDPPUPostProcess& post_processor,
...
void process_image_frames(
    ...
    SCRFDPPUPostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = SCRFDPPUPostProcess(input_width, input_height, 0.5f, 0.45f);
