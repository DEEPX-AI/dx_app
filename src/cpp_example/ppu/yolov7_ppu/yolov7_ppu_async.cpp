...
/**
 * @brief Asynchronous post-processing example for YOLOv7 PPU object detection
model.
 *
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv7PPUResult>>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor =
        YOLOv7PPUPostProcess(input_width, input_height, 0.25f, 0.25f, 0.45f);
