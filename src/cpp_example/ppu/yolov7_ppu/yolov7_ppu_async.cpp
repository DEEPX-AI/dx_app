...
/**
 * @brief Asynchronous post-processing example for YOLOv7 object detection
model.
 *
 ...
 */
void scale_coordinates(YOLOv7PPUResult& detection, const std::vector<int>& pad_xy,
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, std::vector<YOLOv7PPUResult>& detections,
    ...
    while (appQuit->load() == 0) {
        if (wait_queue->size() > 0) {
            ...
            // detections 멤버는 포인터이므로, 벡터를 동적할당하여 복사
            {
                ...
                display_args->detections = new std::vector<YOLOv7PPUResult>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    loopCount = loopTest;
    // std::cout << "=== YOLOV7 Post-Processing ASync Example ===" <<
    // std::endl;
    ...
    if (!imgFile.empty()) {
        ...
        if (profiling_metrics.infer_completed > 0) {
            ...
            std::cout << "[PROFILE] ======== YOLOv7 Async Performance Report ========" << std::endl;
