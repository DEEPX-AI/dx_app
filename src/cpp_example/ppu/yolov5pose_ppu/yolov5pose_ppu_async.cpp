...
/**
 * @brief Asynchronous post-processing example for YOLOv5Pose PPU pose detection model.
 *
...
struct DisplayArgs {
    std::shared_ptr<std::vector<YOLOv5PosePPUResult>> detections;
    std::shared_ptr<cv::Mat> original_frame;

    YOLOv5PosePPUPostProcess *ypp = nullptr;
...
bool process_postprocess(YOLOv5PosePPUPostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<YOLOv5PosePPUResult>& detections_vec) {
    try {
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, const std::vector<YOLOv5PosePPUResult>& detections,
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        std::vector<YOLOv5PosePPUResult> detections_vec;
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv5PosePPUResult>>(std::move(detections_vec));
...
CommandLineArgs parse_command_line(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv5Pose PPU Post-Processing Async Example";
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, YOLOv5PosePPUPostProcess& post_processor,
...
void process_image_frames(
    ...
    YOLOv5PosePPUPostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = YOLOv5PosePPUPostProcess(input_width, input_height, 0.5f, 0.45f);
