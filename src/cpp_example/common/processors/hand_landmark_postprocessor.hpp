...
public:
    ...
    HandLandmarkPostprocessor(int input_width, int input_height)
    ...
    std::vector<HandLandmarkResult> process(
        ...
        if (outputs.size() > 1) {
            const float* hand_data = static_cast<const float*>(outputs[1]->data());
            float handedness_score = hand_data[0];
            result.handedness = (handedness_score > 0.5f) ? "Right" : "Left";
            result.confidence = handedness_score;
        } else {
            result.handedness = "Unknown";
            result.confidence = 1.0f;
        }

        // Parse hand confidence if available (tensor[2])
        if (outputs.size() > 2) {
            const float* conf_data = static_cast<const float*>(outputs[2]->data());
            result.confidence = conf_data[0];
        }
