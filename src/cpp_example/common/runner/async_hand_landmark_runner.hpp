...
public:
    ...
    int run(int argc, char* argv[]) {
        ...
        // Video/camera/RTSP input is supported: each frame is run through the
        // single-model landmark pipeline as-is (no palm-detector crop stage), the
        // same way -i runs on a whole image. Best results still come from a frame
        // that is already centered on a hand (e.g. assets/videos/hand.mp4).
