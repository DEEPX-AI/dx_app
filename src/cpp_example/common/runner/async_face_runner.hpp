...
public:
    ...
    int run(int argc, char* argv[]) {
        ...
        // hand_detection (MediaPipe palm) shares this runner and now supports
        // video/camera/RTSP: the palm detector runs per frame just like
        // face_detection, so no special-casing is needed here.
        ...
        if (is_image) {
            ...
            for (int loop_idx = 0; loop_idx < loopTest && running_ && !g_interrupted(); ++loop_idx) {
                ...
                if (!video.isOpened()) {
                    std::cerr << "[DXAPP] [ERROR] Failed to reopen video: " << args.videoFile << std::endl;
                    break;
                }
            }
        }
...
private:
    ...
    int onAsyncInferenceComplete(
        ...
        printFaceDetectionResults(detections, verbose_, factory_->getTaskType());
     ...
     *  hand_detection reuses this runner (MediaPipe palm) — tag it [HAND], not [FACE]. */
    static void printFaceDetectionResults(const std::vector<FaceDetectionResult>& detections,
                                          bool verbose = false, const std::string& task = "") {
        if (!verbose || detections.empty()) return;
        const char* det_tag = (task == "hand_detection") ? "[HAND]" : "[FACE]";
        std::cout << det_tag << " " << detections.size();
    ...
    void displayThread(IVisualizer<FaceDetectionResult>& visualizer, bool no_display,
        ...
        auto renderArgs = [&](AsyncFaceDisplayArgs& args) {
        ...
        FrameReorderBuffer<AsyncFaceDisplayArgs> reorder(metrics_.max_inflight * 2);
