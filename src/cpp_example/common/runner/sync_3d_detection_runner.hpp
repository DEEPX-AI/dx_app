...
/**
 * @brief Generic synchronous runner for 3D detection models
 * @tparam FactoryT The factory type (must derive from I3DDetectionFactory)
 */
template <typename FactoryT>
class Sync3DDetectionRunner {
    bool verbose_ = false;

public:
    explicit Sync3DDetectionRunner(std::unique_ptr<FactoryT> factory)
...
private:
    ...
    CommandLineArgs parseCommandLine(int argc, char* argv[]) {
        ...
        options.add_options()
            ("m, model_path", "3D detection model file (.dxnn, required)",
    ...
    std::pair<std::vector<std::string>, int> processImagePath(
        ...
        if (fs::is_directory(imageFilePath)) {
            ...
            dxapp::fatal_error("[ERROR] Input file not found: " + imageFilePath);
        }
    ...
    bool processSingleFrame(
        ...
        IPostprocessor<Detection3DResult>& postprocessor,
        IVisualizer<Detection3DResult>& visualizer,
        SyncProfilingMetrics& metrics,
    ...
    void processImageFrames(
        ...
        IPreprocessor& preprocessor, IPostprocessor<Detection3DResult>& postprocessor,
        IVisualizer<Detection3DResult>& visualizer, SyncProfilingMetrics& metrics,
        ...
        for (int i = 0; i < loopTest; ++i) {
            ...
            if (!processSingleFrame(img, display_image, ie, preprocessor, postprocessor,
                                    ...
                                    i, frameDumpPath, false, currentImagePath)) break;
            processCount++;
            if (!no_display) {
