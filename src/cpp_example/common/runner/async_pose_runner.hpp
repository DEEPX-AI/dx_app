...
public:
    ...
    int run(int argc, char* argv[]) {
        ...
        // Image-only task (object pose estimation, e.g. DOPE): -v/-c/-r are accepted as
        // CLI options, but this task has no video/camera example — reject stream input.
        // NOTE: regular pose_estimation / keypoint_detection share this runner and DO
        // support video, so guard only the object_pose_estimation task string.
        {
            ...
            if (_io_task == "object_pose_estimation" && _io_stream) {
        ...
        if (!is_image) {
            ...
            if (args.verbose) {
                ...
                if (!args.videoFile.empty()) std::cout << "[DXAPP] [INFO] Total frames: " << total_frames << std::endl;
                std::cout << std::endl;
            }
            if (args.saveMode) {
                std::string run_name;
...
private:
    ...
    void displayThread(IVisualizer<PoseResult>& visualizer, bool no_display,
        ...
        auto renderArgs = [&](AsyncPoseDisplayArgs& args) {
        ...
        FrameReorderBuffer<AsyncPoseDisplayArgs> reorder(metrics_.max_inflight * 2);
