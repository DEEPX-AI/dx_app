...
struct AsyncOBBDisplayArgs {
    std::shared_ptr<std::vector<Detection3DResult>> detections;
...
class Async3DDetectionRunner {
    bool verbose_ = false;

public:
    explicit Async3DDetectionRunner(std::unique_ptr<FactoryT> factory)
    ...
    int run(int argc, char* argv[]) {
        ...
        // SFA3D BEV preprocessor emits CV_32F (4 bytes/elem) while GetInputSize()
        // may report the UINT8 byte count — size the async buffer for float payloads.
        const size_t preprocess_bytes = static_cast<size_t>(
            input_width * input_height * 3 * sizeof(float));
        const size_t buf_size = std::max(input_size, preprocess_bytes);
        std::vector<std::vector<uint8_t>> input_buffers(
            ASYNC_BUFFER_SIZE, std::vector<uint8_t>(buf_size));
        ...
        ie.RegisterCallback([this, postprocessor](dxrt::TensorPtrs& outputs, void* user_data) -> int {  // NOSONAR(cpp:S5008)
            ...
            std::vector<Detection3DResult> detections;
            ...
            display_args.detections = std::make_shared<std::vector<Detection3DResult>>(std::move(detections));
        ...
        if (is_image) {
            ...
            for (int i = 0; i < loopTest && running_ && !g_interrupted(); ++i) {
                ...
                cv::Mat img = dxapp::loadDisplayFrame(
                    imageFiles[i % imageFiles.size()],
                    preprocessor->getInputHeight(),
                    preprocessor->getInputWidth());
                ...
                auto ud = std::make_unique<AsyncUserData>(AsyncUserData{
                    display_image.clone(), ctx, std::move(save_path), {},
                    imageFiles[i % imageFiles.size()]});
                metrics_.waitForSlot();
                {
                    std::lock_guard<std::mutex> lock(metrics_.metrics_mutex);
        ...
        if (is_image && processCount > 0) {
            const int expected = processCount;
            const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(30);
            while (running_ && std::chrono::steady_clock::now() < deadline) {
                int completed = 0;
                {
                    std::lock_guard<std::mutex> lock(metrics_.metrics_mutex);
                    completed = metrics_.infer_completed;
                }
                if (completed >= expected && display_queue_.empty()) {
                    break;
                }
                if (!args.no_display) {
                    pollDisplay();
        ...
        if (displayThr.joinable()) displayThr.join();
...
private:
    ...
    CommandLineArgs parseCommandLine(int argc, char* argv[]) {
        ...
        options.add_options()
            ...
            ("calib-dir", "Directory of {frame_id}.txt calib files paired with -i stems",
             cxxopts::value<std::string>())
            ("image2-dir", "Directory of {frame_id}.png/.jpg camera images paired with -i stems",
             cxxopts::value<std::string>())
            ("h, help", "print usage");
        auto cmd = options.parse(argc, argv);
        if (cmd.count("help")) { std::cout << options.help() << std::endl; exit(0); }
        if (cmd.count("calib-dir")) {
            setCalibDirOverride(cmd["calib-dir"].as<std::string>());
        }
        if (cmd.count("image2-dir")) {
            setImage2DirOverride(cmd["image2-dir"].as<std::string>());
        }
        return args;
    }
    ...
    std::pair<std::vector<std::string>, int> processImagePath(const std::string& imageFilePath, int loopTest) {
        ...
        } else { dxapp::fatal_error("[ERROR] Input file not found: " + imageFilePath); }
