...
public:
    ...
    int run(int argc, char* argv[]) {
        ...
        if (is_image) {
            ...
            if (is_dir_input) {
                if (args.verbose) {
                    std::cout << "[DXAPP] [INFO] Found " << images_per_loop << " images in directory: "
                          << args.imageFilePath << std::endl;
                }
            }
            if (args.verbose) {
                std::cout << "[DXAPP] [INFO] Image input (" << images_per_loop << " files)" << std::endl;
                std::cout << "[DXAPP] [INFO] Images per loop: " << images_per_loop << std::endl;
                std::cout << "[DXAPP] [INFO] Loop count: " << user_loop_count << std::endl;
                std::cout << std::endl;
            }
        }
        ...
        if (!is_image) {
            ...
            if (args.verbose) {
                ...
                std::cout << "[DXAPP] [INFO] Input source resolution (WxH): "
                << vid_frame_width << "x" << vid_frame_height << std::endl;
                std::cout << "[DXAPP] [INFO] Input source FPS: " << std::fixed
                << std::setprecision(2) << vid_fps << std::endl;
            }
            if (!args.videoFile.empty()) {
                if (args.verbose) {
                    std::cout << "[DXAPP] [INFO] Total frames: " << vid_total_frames << std::endl;
                }
            }
            if (loopTest > 1) {
                if (args.verbose) {
        ...
        if (g_interrupted().load()) {
            std::cout << "[DXAPP] [INFO] Interrupted by user (Ctrl+C)" << std::endl;
        }
        ...
        if (is_image) {
            ...
            if (user_loop_count > 1) {
                std::cout << "\n[DXAPP] [INFO] Average performance over "
...
private:
    ...
    static void finalizeVideoWriter(int loop_idx, bool saveMode,
        ...
        if (loop_idx == 0 && saveMode && writer.isOpened()) {
            ...
            if (!videoSavePathResolved.empty()) {
                if (verbose) {
                    std::cout << "\n[DXAPP] [INFO] Saved output video: " << videoSavePathResolved << std::endl;
                }
            }
        }
    }
    ...
    bool processSingleFrame(
        ...
        if (!dumpTensorsDir.empty()) {
            ...
            if (verbose_) {
                std::cout << "\n[DXAPP] [INFO] Saved input & output tensors: "
        ...
        try {
            ...
            if (dumpTensorsDir.empty()) {
                ...
                if (verbose_) {
                    std::cout << "[DXAPP] [INFO] Auto-dumped tensors on exception to: "
        ...
        for (int loop_idx = 0; loop_idx < user_loop_count && !g_interrupted().load(); ++loop_idx) {
            if (multi_loop) {
                if (verbose_) {
                    ...
                    std::cout << "[DXAPP] [INFO] Loop " << (loop_idx + 1) << "/" << user_loop_count << std::endl;
            ...
            for (int img_idx = 0; img_idx < images_per_loop && !g_interrupted().load(); ++img_idx) {
                ...
                if (loop_idx == 0 || (is_dir_input && !multi_loop)) {
                    if (verbose_) {
                        ...
                        std::cout << "[DXAPP] [INFO] Image resolution (WxH): "
