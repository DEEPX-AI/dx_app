...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        if (!process_postprocess(*args->dlpp, outputs, segmentation_result)) {
            appQuit->store(1);
            continue;
        }
        ...
        d_args->segmentation_result = std::make_shared<DeepLabv3Result>(std::move(segmentation_result));
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, DeepLabv3PostProcess& post_processor,
...
void process_image_frames(
    ...
    DeepLabv3PostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
