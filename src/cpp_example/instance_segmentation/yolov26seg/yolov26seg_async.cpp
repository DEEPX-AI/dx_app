...
/**
 * @brief Asynchronous post-processing example for yolov26-seg instance segmentation model.
 *
...
struct DisplayArgs {
    std::shared_ptr<std::vector<YOLOv26SegResult>> detections;
    std::shared_ptr<cv::Mat> original_frame;

    YOLOv26SegPostProcess *ypp = nullptr;
...
bool process_postprocess(YOLOv26SegPostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<YOLOv26SegResult>& detections_vec) {
    try {
 ...
 */
void draw_segmentation_mask(cv::Mat& result, const YOLOv26SegResult& detection,
                            const std::vector<int>& pad_xy, const std::vector<float>& ratio,
...
void draw_detection_box(cv::Mat& result, const std::vector<float>& box,
                        const YOLOv26SegResult& detection, const cv::Scalar& color) {
...
/**
 * @brief Visualize YOLOv26 segmentation results by drawing bounding boxes and masks.
 * @param frame Original image
 * @param results YOLOv26 segmentation results
 * @param pad_xy Padding offsets [x, y]
 * @param ratio Scale ratios [x, y]
 * @param alpha Blending factor for mask overlay (0.0 = original image, 1.0 = only mask)
 * @return Visualized image
 */
cv::Mat draw_yolov26_segmentation(const cv::Mat& frame, std::vector<YOLOv26SegResult>& results,
                                 const std::vector<int>& pad_xy, const std::vector<float>& ratio,
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        std::vector<YOLOv26SegResult> detections_vec;
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv26SegResult>>(std::move(detections_vec));
...
void display_thread_func(SafeQueue<std::shared_ptr<DisplayArgs>> *display_queue,
    ...
    while (appQuit->load() == 0 || !display_queue->empty()) {
        ...
        auto processed_frame =
            draw_yolov26_segmentation(*args->original_frame, *args->detections, args->pad_xy, args->ratio);
...
CommandLineArgs parse_command_line(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv26-Seg Post-Processing Async Example";
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, YOLOv26SegPostProcess& post_processor,
...
void process_image_frames(
    ...
    YOLOv26SegPostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = YOLOv26SegPostProcess(input_width, input_height, 0.3f, ie.IsOrtConfigured());
