...
/**
 * @brief Asynchronous post-processing example for YOLOv9 object detection
model.
 *
 ...
 */
void scale_coordinates(YOLOv9Result& detection, const std::vector<int>& pad_xy,
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, std::vector<YOLOv9Result>& detections,
...
class SafeQueue {
   ...
   public:
    ...
    void push(DetectionArgs* item, int timeout_ms = 1000) {
...
void post_process_thread_func(SafeQueue* wait_queue, std::queue<DisplayArgs*>* display_queue,
                              std::atomic<int>* appQuit, std::atomic<int>* forceQuit) {
    ...
    while (appQuit->load() == 0) {
        if (wait_queue->size() > 0) {
            ...
            std::vector<YOLOv9Result> detections_vec;
            try {
            ...
            // detections 멤버는 포인터이므로, 벡터를 동적할당하여 복사
            {
                ...
                display_args->detections = new std::vector<YOLOv9Result>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv9 Post-Processing Async Example";
    ...
    loopCount = loopTest;
    // std::cout << "=== YOLOV9 Post-Processing ASync Example ===" <<
    // std::endl;
    ...
    auto post_processor =
        YOLOv9PostProcess(input_width, input_height, 0.3f, 0.45f, ie.IsOrtConfigured());
    ...
    std::thread post_process_thread(post_process_thread_func, &wait_queue, &display_queue, &appQuit,
                                    &forceQuit);
    ...
    if (!imgFile.empty()) {
        ...
        while (processCount < loopCount && forceQuit.load() < 1) {
            std::this_thread::sleep_for(std::chrono::microseconds(10));
        }
        appQuit.store(1);
        ...
        if (profiling_metrics.infer_completed > 0) {
            ...
            std::cout << "[PROFILE] ======== YOLOv9 Async Performance Report ========" << std::endl;
        ...
        if (forceQuit.load() == 1) {
            std::cout << "[WARNING] Application terminated early due to an error during processing."
                      << std::endl;
            return -1;
        }
