...
/**
 * @brief Asynchronous post-processing example for YOLOv5 object detection
model.
 *
...
struct DisplayArgs {
    std::shared_ptr<std::vector<YOLOv5Result>> detections;
    std::shared_ptr<cv::Mat> original_frame;

    YOLOv5PostProcess *ypp = nullptr;
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv5Result>>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv5 Post-Processing Async Example";
    ...
    auto post_processor =
        YOLOv5PostProcess(input_width, input_height, 0.25f, 0.25f, 0.45f, ie.IsOrtConfigured());
