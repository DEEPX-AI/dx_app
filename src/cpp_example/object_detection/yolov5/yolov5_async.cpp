...
/**
 * @brief Asynchronous post-processing example for YOLOv5s object detection
model.
 *
 ...
 */
void scale_coordinates(YOLOv5Result& detection, const std::vector<int>& pad_xy,
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, std::vector<YOLOv5Result>& detections,
...
void post_process_thread_func(SafeQueue* wait_queue, std::queue<DisplayArgs*>* display_queue,
    ...
    while (appQuit->load() == 0) {
        if (wait_queue->size() > 0) {
            ...
            // Start postprocess timing - aligned tensor processing is now handled inside
            // postprocess
            ...
            // detections 멤버는 포인터이므로, 벡터를 동적할당하여 복사
            {
                ...
                display_args->detections = new std::vector<YOLOv5Result>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv5s Post-Processing Async Example";
    ...
    loopCount = loopTest;
    // std::cout << "=== YOLOV5 Post-Processing ASync Example ===" <<
    // std::endl;
    ...
    auto post_processor =
        YOLOv5PostProcess(input_width, input_height, 0.25, 0.3, 0.45, ie.IsOrtConfigured());
