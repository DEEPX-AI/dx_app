...
/**
 * @brief Asynchronous post-processing example for YOLOv5 object detection model.
 *
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv5Result>>(std::move(detections_vec));
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, YOLOv5PostProcess& post_processor,
...
void process_image_frames(
    ...
    YOLOv5PostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
