...
/**
 * @brief Asynchronous post-processing example for YOLOv26 object detection model.
 *
...
bool process_postprocess(YOLOv26PostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<YOLOv26Result>& detections_vec) {
    try {
...
void draw_detection_box(cv::Mat& result, const std::vector<float>& box,
                        const YOLOv26Result& detection, const cv::Scalar& color) {
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, const std::vector<YOLOv26Result>& detections,
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv26Result>>(std::move(detections_vec));
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, YOLOv26PostProcess& post_processor,
...
void process_image_frames(
    ...
    YOLOv26PostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = YOLOv26PostProcess(input_width, input_height, 0.3f, ie.IsOrtConfigured());
