...
/**
 * @brief Asynchronous post-processing example for YOLOv7 object detection model.
 *
...
bool process_postprocess(YOLOv7PostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<YOLOv7Result>& detections_vec) {
    try {
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv7Result>>(std::move(detections_vec));
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, YOLOv7PostProcess& post_processor,
...
void process_image_frames(
    ...
    YOLOv7PostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
