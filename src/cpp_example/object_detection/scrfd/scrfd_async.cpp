...
struct DisplayArgs {
    std::shared_ptr<std::vector<SCRFDResult>> detections;
    std::shared_ptr<cv::Mat> original_frame;

    SCRFDPostProcess *spp = nullptr;
...
bool process_postprocess(SCRFDPostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<SCRFDResult>& detections_vec) {
    try {
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, const std::vector<SCRFDResult>& detections,
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        std::vector<SCRFDResult> detections_vec;
        ...
        d_args->detections = std::make_shared<std::vector<SCRFDResult>>(std::move(detections_vec));
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, SCRFDPostProcess& post_processor,
...
void process_image_frames(
    ...
    SCRFDPostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = SCRFDPostProcess(input_width, input_height, 0.5f, 0.45f, ie.IsOrtConfigured());
