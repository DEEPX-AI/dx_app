 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, std::vector<SCRFDResult>& detections,
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        d_args->detections = std::make_shared<std::vector<SCRFDResult>>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor =
        SCRFDPostProcess(input_width, input_height, 0.5f, 0.45f, ie.IsOrtConfigured());
