...
/**
 * @brief Asynchronous post-processing example for SCRFD face detection
model.
 *
...
void make_letterbox_image(const cv::Mat& image, cv::Mat& preprocessed_image,
    ...
    cv::copyMakeBorder(resized_image, preprocessed_image, top, bottom, left,
                       right, cv::BORDER_CONSTANT, cv::Scalar(114));
}
 ...
 */
void scale_coordinates(SCRFDResult& detection, const std::vector<int>& pad_xy,
                       const float letterbox_scale) {
    detection.box[0] =
        (detection.box[0] - static_cast<float>(pad_xy[0])) / letterbox_scale;
    detection.box[1] =
        (detection.box[1] - static_cast<float>(pad_xy[1])) / letterbox_scale;
    detection.box[2] =
        (detection.box[2] - static_cast<float>(pad_xy[0])) / letterbox_scale;
    detection.box[3] =
        (detection.box[3] - static_cast<float>(pad_xy[1])) / letterbox_scale;
    for (size_t i = 0; i < detection.landmarks.size(); i += 2) {
        detection.landmarks[i] =
            (detection.landmarks[i] - static_cast<float>(pad_xy[0])) /
            letterbox_scale;
        detection.landmarks[i + 1] =
            (detection.landmarks[i + 1] - static_cast<float>(pad_xy[1])) /
            letterbox_scale;
    }
}
...
cv::Mat draw_detections(const cv::Mat& frame,
    ...
    for (auto& detection : detections) {
        ...
        cv::putText(result, conf_text, text_pos, font_face, font_scale,
                    cv::Scalar(255, 255, 255), thickness);

        // Draw landmarks if requested
        if (!detection.landmarks.empty()) {
            for (size_t i = 0; i < detection.landmarks.size(); i +=2) {
                cv::Point2f landmark(detection.landmarks[i],
                                     detection.landmarks[i + 1]);
 ...
 */
void post_process_thread_func(SafeQueue* wait_queue,
    ...
    while (appQuit->load() == 0) {
        if (wait_queue->size() > 0) {
            ...
            auto detections_vec = args->spp->postprocess(outputs);
            ...
            if (args->metrics) {
                ...
                auto dt = std::chrono::duration<double>(t1 - args->metrics->inflight_last_ts).count();
            ...
            // detections 멤버는 포인터이므로, 벡터를 동적할당하여 복사
            {
                ...
                display_args->detections = new std::vector<SCRFDResult>(detections_vec);
                ...
                // spp: pointer to SCRFDPostProcess instance
                display_args->spp = args->spp;
 ...
 */
void display_thread_func(std::queue<DisplayArgs*>* display_queue,
...
int main(int argc, char* argv[]) {
    ...
    std::string app_name = "SCRFD Post-Processing Async Example";
    cxxopts::Options options(app_name, app_name + " application usage ");
    options.add_options()("m, model_path",
                          "object detection model file (.dxnn, required)",
        ...
        "v, video_path", "input video file path(mp4, mov, avi ..., required)",
        ...
        cxxopts::value<bool>(fps_only)->default_value("false"))("h, help",
                                                                "print usage");
    ...
    auto post_processor = SCRFDPostProcess(input_width, input_height, 0.6,
                                           0.45, ie.IsOrtConfigured());
    ...
    std::thread post_process_thread(post_process_thread_func, &wait_queue,
                                    &display_queue, &appQuit);
    cv::VideoWriter writer;
    std::vector<int> pad_xy{0, 0};
    float scale_factor = 1.f;
    std::thread display_thread(display_thread_func, &display_queue, &appQuit,
                               &writer, &pad_xy, &scale_factor);

    // 이미지 파일이 있을 때 loopTest가 1이면 100으로 자동 설정
    if (!imgFile.empty()) {
        if (loopTest == 1) {
            std::cout
                << "loopTest is set to 100 when an image file is provided."
                << std::endl;
            loopCount = 100;
            loopTest = 100;
        }
        ...
        std::cout << "[INFO] Image resolution (WxH): " << image.cols << "x" << image.rows << std::endl;
        ...
        int letterbox_pad_x = std::max(
            0.f,
            (post_processor.get_input_width() - image.cols * scale_factor) / 2);
        int letterbox_pad_y = std::max(0.f, (post_processor.get_input_height() -
        ...
        do {
            ...
            cv::Mat preprocessed_image =
                cv::Mat(post_processor.get_input_height(),
                        post_processor.get_input_width(), CV_8UC3,
                        input_buffers[index].data());
            make_letterbox_image(image, preprocessed_image, cv::COLOR_BGR2RGB,
                                 pad_xy);
            ...
            auto req_id = ie.RunAsync(preprocessed_image.data, nullptr,
            ...
            // Update inflight tracking
            {
                ...
                if (profiling_metrics.first_inference) {
                    ...
                    auto dt = std::chrono::duration<double>(t2 - profiling_metrics.inflight_last_ts).count();
        ...
        if (profiling_metrics.infer_completed > 0) {
            ...
            auto inflight_time_window = std::chrono::duration<double>(
                profiling_metrics.infer_last_ts - profiling_metrics.infer_first_ts).count();
            ...
            std::cout << "[PROFILE] ======== SCRFD Async Performance Report ========" << std::endl;
            std::cout << "[PROFILE] Preprocess latency = " << std::fixed << std::setprecision(2) 
                      << avg_pre << " ms (" << std::setprecision(1) << pre_fps << " FPS)" << std::endl;
            std::cout << "[PROFILE] Inference latency = " << std::setprecision(2) << avg_inf 
                      << " ms (" << std::setprecision(1) << inf_fps << " FPS) | Inference throughput = " 
                      << std::setprecision(1) << infer_tp << " FPS | Inflight avg = " 
                      << std::setprecision(1) << inflight_avg << ", max = " << profiling_metrics.inflight_max << std::endl;
            std::cout << "[PROFILE] Postprocess latency = " << std::setprecision(2) << avg_post 
                      << " ms (" << std::setprecision(1) << post_fps << " FPS)" << std::endl;
                      
            if (!fps_only) {
                double avg_render = profiling_metrics.sum_render / profiling_metrics.infer_completed;
        ...
        double overall_fps = processCount / (std::chrono::duration_cast<std::chrono::milliseconds>(e - s).count() / 1000.0);
        std::cout << "[SUCCESS] End-to-End FPS : " << std::setprecision(1) << overall_fps << std::endl;
    } else if (!videoFile.empty()) {
        std::cout << "loopTest is set to 1 when a video file is provided."
                  << std::endl;
        loopTest = 1;
        ...
        std::cout << "[INFO] Video resolution (WxH): " << video_width << "x" << video_height << std::endl;
        std::cout << "[INFO] Total frames: " << total_frames << std::endl;
        std::cout << "[INFO] Video FPS: " << std::fixed << std::setprecision(2) << video_fps << std::endl;

        if (fps_only) {
            std::cout
                << "Processing video stream... Only FPS will be displayed."
                << std::endl;
        }
        if (saveVideo) {
            writer = cv::VideoWriter(
                "result.mp4", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'),
                video.get(cv::CAP_PROP_FPS),
                cv::Size(SHOW_WINDOW_SIZE_W, SHOW_WINDOW_SIZE_H));
            if (!writer.isOpened()) {
                std::cerr << "[ERROR] Failed to open video writer."
                          << std::endl;
                exit(1);
            }
        }
        ...
        scale_factor = std::min(post_processor.get_input_width() /
                                    static_cast<float>(SHOW_WINDOW_SIZE_W),
                                post_processor.get_input_height() /
                                    static_cast<float>(SHOW_WINDOW_SIZE_H));
        int letterbox_pad_x =
            std::max(0.f, (post_processor.get_input_width() -
                           SHOW_WINDOW_SIZE_W * scale_factor) /
                              2);
        int letterbox_pad_y =
            std::max(0.f, (post_processor.get_input_height() -
                           SHOW_WINDOW_SIZE_H * scale_factor) /
                              2);
        pad_xy = {letterbox_pad_x, letterbox_pad_y};
        ...
        do {
            ...
            auto req_id = ie.RunAsync(preprocessed_image.data, nullptr, output_buffers[index].data());
            auto t2 = std::chrono::high_resolution_clock::now();
