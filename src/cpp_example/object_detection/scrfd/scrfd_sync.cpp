...
/**
 * @brief Synchronous post-processing example for SCRFD face detection
model.
 *
...
int main(int argc, char* argv[]) {
    ...
    std::string app_name = "SCRFD Post-Processing Sync Example";
    ...
    if (!imgFile.empty()) {
        ...
        do {
            ...
            if (!outputs.empty()) {
                ...
                auto result_image =
                    draw_detections(image, detections, pad_xy, scale_factor);
                if (loopTest == 1) {
        ...
        scale_factor = std::min(
            post_processor.get_input_width() / static_cast<float>(frame_width),
            post_processor.get_input_height() /
                static_cast<float>(frame_height));
        int letterbox_pad_x = std::max(0.f, (post_processor.get_input_width() -
        ...
        do {
            ...
            cv::Mat preprocessed_image(post_processor.get_input_height(),
                                       post_processor.get_input_width(),
                                       CV_8UC3);
            make_letterbox_image(image, preprocessed_image, cv::COLOR_BGR2RGB,
                                 pad_xy);
            auto outputs = ie.Run(preprocessed_image.data);
