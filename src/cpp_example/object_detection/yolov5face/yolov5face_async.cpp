...
/**
 * @brief Asynchronous post-processing example for YOLOv5Face face detection model.
 *
...
struct DisplayArgs {
    std::shared_ptr<std::vector<YOLOv5FaceResult>> detections;
    std::shared_ptr<cv::Mat> original_frame;

    YOLOv5FacePostProcess *ypp = nullptr;
...
bool process_postprocess(YOLOv5FacePostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<YOLOv5FaceResult>& detections_vec) {
    try {
...
/**
 * @brief Transform detection boxes and landmarks from model input coordinates to original image coordinates.
 ...
 */
void transform_box_to_original(YOLOv5FaceResult& detection, const std::vector<int>& pad_xy,
                               const std::vector<float>& ratio, int orig_width, int orig_height) {
    // Remove padding and scale back to original coordinates
    detection.box[0] = (detection.box[0] - static_cast<float>(pad_xy[0])) / ratio[0];
    detection.box[1] = (detection.box[1] - static_cast<float>(pad_xy[1])) / ratio[1];
    detection.box[2] = (detection.box[2] - static_cast<float>(pad_xy[0])) / ratio[0];
    detection.box[3] = (detection.box[3] - static_cast<float>(pad_xy[1])) / ratio[1];

    // Clamp to image boundaries
    detection.box[0] = std::max(0.0f, std::min(static_cast<float>(orig_width), detection.box[0]));
    detection.box[1] = std::max(0.0f, std::min(static_cast<float>(orig_height), detection.box[1]));
    detection.box[2] = std::max(0.0f, std::min(static_cast<float>(orig_width), detection.box[2]));
    detection.box[3] = std::max(0.0f, std::min(static_cast<float>(orig_height), detection.box[3]));

    // Transform landmarks
    for (size_t i = 0; i < detection.landmarks.size(); i += 2) {
        detection.landmarks[i] =
            (detection.landmarks[i] - static_cast<float>(pad_xy[0])) / ratio[0];
        detection.landmarks[i + 1] =
            (detection.landmarks[i + 1] - static_cast<float>(pad_xy[1])) / ratio[1];
    }
}
...
cv::Mat draw_detections(const cv::Mat& frame, std::vector<YOLOv5FaceResult>& detections,
    ...
    for (auto& detection : detections) {
        transform_box_to_original(detection, pad_xy, ratio, frame.cols, frame.rows);
        // Draw bounding box
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        std::vector<YOLOv5FaceResult> detections_vec;
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv5FaceResult>>(std::move(detections_vec));
...
CommandLineArgs parse_command_line(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv5Face Post-Processing Async Example";
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, YOLOv5FacePostProcess& post_processor,
...
void process_image_frames(
    ...
    YOLOv5FacePostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = YOLOv5FacePostProcess(input_width, input_height, 0.25f, 0.25f, 0.45f, ie.IsOrtConfigured());
