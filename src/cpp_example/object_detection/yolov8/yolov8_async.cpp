...
/**
 * @brief Asynchronous post-processing example for YOLOv8n object detection
model.
 *
 ...
 */
void scale_coordinates(YOLOv8Result& detection, const std::vector<int>& pad_xy,
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, std::vector<YOLOv8Result>& detections,
    ...
    while (appQuit->load() == 0) {
        if (wait_queue->size() > 0) {
            ...
            // detections 멤버는 포인터이므로, 벡터를 동적할당하여 복사
            {
                ...
                display_args->detections = new std::vector<YOLOv8Result>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv8n Post-Processing Async Example";
    ...
    loopCount = loopTest;
    // std::cout << "=== YOLOV8 Post-Processing ASync Example ===" <<
    // std::endl;
    ...
    auto post_processor =
        YOLOv8PostProcess(input_width, input_height, 0.3f, 0.45f, ie.IsOrtConfigured());
    ...
    if (!imgFile.empty()) {
        ...
        if (profiling_metrics.infer_completed > 0) {
            ...
            std::cout << "[PROFILE] ======== YOLOv8 Async Performance Report ========" << std::endl;
