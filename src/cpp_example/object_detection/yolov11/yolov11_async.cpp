...
/**
 * @brief Asynchronous post-processing example for YOLOv11 object detection model.
 *
...
bool process_postprocess(YOLOv11PostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<YOLOv11Result>& detections_vec) {
    try {
...
void draw_detection_box(cv::Mat& result, const std::vector<float>& box,
                        const YOLOv11Result& detection, const cv::Scalar& color) {
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, const std::vector<YOLOv11Result>& detections,
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv11Result>>(std::move(detections_vec));
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, YOLOv11PostProcess& post_processor,
...
void process_image_frames(
    ...
    YOLOv11PostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = YOLOv11PostProcess(input_width, input_height, 0.3f, 0.45f, ie.IsOrtConfigured());
