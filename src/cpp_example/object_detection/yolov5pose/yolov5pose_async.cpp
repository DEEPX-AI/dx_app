...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv5PoseResult>>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor =
        YOLOv5PosePostProcess(input_width, input_height, 0.5f, 0.5f, 0.45f, ie.IsOrtConfigured());
