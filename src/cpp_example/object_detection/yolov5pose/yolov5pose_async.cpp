...
/**
 * @brief Asynchronous post-processing example for YOLOv5Pose pose detection model.
 *
...
struct DisplayArgs {
    std::shared_ptr<std::vector<YOLOv5PoseResult>> detections;
    std::shared_ptr<cv::Mat> original_frame;

    YOLOv5PosePostProcess *ypp = nullptr;
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv5PoseResult>>(std::move(detections_vec));
...
void process_image_frames(
    ...
    YOLOv5PosePostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
