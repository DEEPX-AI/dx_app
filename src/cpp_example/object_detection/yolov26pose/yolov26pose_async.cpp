...
/**
 * @brief Asynchronous post-processing example for YOLOv26Pose pose detection model.
 *
...
struct DisplayArgs {
    std::shared_ptr<std::vector<YOLOv26PoseResult>> detections;
    std::shared_ptr<cv::Mat> original_frame;

    YOLOv26PosePostProcess *ypp = nullptr;
...
bool process_postprocess(YOLOv26PosePostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<YOLOv26PoseResult>& detections_vec) {
    try {
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, const std::vector<YOLOv26PoseResult>& detections,
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        std::vector<YOLOv26PoseResult> detections_vec;
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv26PoseResult>>(std::move(detections_vec));
...
CommandLineArgs parse_command_line(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv26Pose Post-Processing Async Example";
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, YOLOv26PosePostProcess& post_processor,
...
void process_image_frames(
    ...
    YOLOv26PosePostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = YOLOv26PosePostProcess(input_width, input_height, 0.5f, ie.IsOrtConfigured());
