...
/**
 * @brief Asynchronous post-processing example for YOLOv12 object detection model.
 *
...
struct DisplayArgs {
    std::shared_ptr<std::vector<YOLOv12Result>> detections;
    std::shared_ptr<cv::Mat> original_frame;

    YOLOv12PostProcess *ypp = nullptr;
...
bool process_postprocess(YOLOv12PostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<YOLOv12Result>& detections_vec) {
    try {
...
void draw_detection_box(cv::Mat& result, const std::vector<float>& box,
                        const YOLOv12Result& detection, const cv::Scalar& color) {
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, const std::vector<YOLOv12Result>& detections,
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        std::vector<YOLOv12Result> detections_vec;
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv12Result>>(std::move(detections_vec));
...
CommandLineArgs parse_command_line(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv12 Post-Processing Async Example";
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, YOLOv12PostProcess& post_processor,
...
void process_image_frames(
    ...
    YOLOv12PostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = YOLOv12PostProcess(input_width, input_height, 0.3f, 0.45f, ie.IsOrtConfigured());
