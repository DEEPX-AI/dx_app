...
/**
 * @brief Asynchronous post-processing example for YOLOv26Obb object detection model.
 *
...
struct DisplayArgs {
    std::shared_ptr<std::vector<YOLOv26ObbResult>> detections;
    std::shared_ptr<cv::Mat> original_frame;

    YOLOv26ObbPostProcess *ypp = nullptr;
...
bool process_postprocess(YOLOv26ObbPostProcess& post_processor, const std::vector<std::shared_ptr<dxrt::Tensor>>& outputs, std::vector<YOLOv26ObbResult>& detections_vec) {
    try {
...
/**
 * @brief Convert OBB center/size from letterboxed coordinates back to
 * display image coordinates.
 * @param box OBB layout: [cx, cy, w, h, angle]
 * @param pad_xy [x, y] vector for padding size used in letterbox
 * @param ratio [x, y] vector for scale ratio used in letterbox
 */
void transform_box_to_original(std::vector<float>& box, const std::vector<int>& pad_xy,
                               const std::vector<float>& ratio, int /*orig_width*/,
                               int /*orig_height*/) {
    if (box.size() < 4) return;

    // Remove padding and scale back to display-image coordinates
    box[0] = (box[0] - static_cast<float>(pad_xy[0])) / ratio[0];  // cx
    box[1] = (box[1] - static_cast<float>(pad_xy[1])) / ratio[1];  // cy
    box[2] = box[2] / ratio[0];                                    // w
    box[3] = box[3] / ratio[1];                                    // h
}

/**
 * @brief Draw a single OBB detection with label on the image.
...
void draw_detection_box(cv::Mat& result, const std::vector<float>& box,
                        const YOLOv26ObbResult& detection, const cv::Scalar& color) {
    if (box.size() < 5) return;

    float cx = box[0];
    float cy = box[1];
    float w = box[2];
    float h = box[3];
    float angle = box[4];  // radians

    float cos_v = std::cos(angle);
    float sin_v = std::sin(angle);

    cv::Point2f ctr(cx, cy);
    cv::Point2f vec1(w * 0.5f * cos_v, w * 0.5f * sin_v);
    cv::Point2f vec2(-h * 0.5f * sin_v, h * 0.5f * cos_v);

    cv::Point2f p1 = ctr + vec1 + vec2;
    cv::Point2f p2 = ctr + vec1 - vec2;
    cv::Point2f p3 = ctr - vec1 - vec2;
    cv::Point2f p4 = ctr - vec1 + vec2;

    std::vector<cv::Point> poly = {p1, p2, p3, p4};
    cv::polylines(result, poly, true, color, 2);
    ...
    int min_x = std::min(std::min(poly[0].x, poly[1].x), std::min(poly[2].x, poly[3].x));
    int min_y = std::min(std::min(poly[0].y, poly[1].y), std::min(poly[2].y, poly[3].y));
    cv::Point label_pt(min_x, min_y - 10);

    cv::Point bg_tl(label_pt.x, label_pt.y - label_size.height);
    cv::Point bg_br(label_pt.x + label_size.width, label_pt.y + baseline);
    cv::rectangle(result, bg_tl, bg_br, cv::Scalar(0, 0, 0), cv::FILLED);

    cv::putText(result, label, label_pt, font_face, font_scale, cv::Scalar(255, 255, 255),
                thickness);
}

/**
 * @brief Visualize OBB detection results on the image by drawing rotated
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, const std::vector<YOLOv26ObbResult>& detections,
...
void post_process_thread_func(SafeQueue<std::shared_ptr<DetectionArgs>> *wait_queue,
    ...
    while (appQuit->load() == 0) {
        ...
        std::vector<YOLOv26ObbResult> detections_vec;
        ...
        d_args->detections = std::make_shared<std::vector<YOLOv26ObbResult>>(std::move(detections_vec));
...
CommandLineArgs parse_command_line(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOv26Obb Post-Processing Async Example";
...
void submit_frame_for_inference(
    ...
    dxrt::InferenceEngine& ie, YOLOv26ObbPostProcess& post_processor,
...
void process_image_frames(
    ...
    YOLOv26ObbPostProcess& post_processor, SafeQueue<std::shared_ptr<DetectionArgs>>& wait_queue,
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor = YOLOv26ObbPostProcess(input_width, input_height, 0.3f, ie.IsOrtConfigured());
