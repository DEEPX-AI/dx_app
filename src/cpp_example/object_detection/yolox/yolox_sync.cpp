...
/**
 * @brief Synchronous post-processing example for YOLOX object detection
model.
 *
...
int main(int argc, char* argv[]) {
    ...
    std::string app_name = "YOLOX Post-Processing Sync Example";
    ...
    // std::cout << "=== YOLOX Post-Processing ASync Example ===" <<
    // std::endl;
    ...
    auto post_processor = YOLOXPostProcess(input_width, input_height, 0.25f,
                                           0.3f, 0.45f, ie.IsOrtConfigured());
    ...
    if (!imgFile.empty()) {
        ...
        do {
            ...
            make_letterbox_image(image, preprocessed_image, cv::COLOR_BGR2RGB,
                                 pad_xy);
            cv::imwrite("preprocessed.jpg", preprocessed_image);
