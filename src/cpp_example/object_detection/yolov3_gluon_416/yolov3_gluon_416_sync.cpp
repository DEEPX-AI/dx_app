/**
 * @file yolov3_gluon_416_sync.cpp
 * @brief YOLOv3_gluon_416 synchronous inference example
 * 
 * Uses abstract factory pattern for clean separation of concerns.
 */

#include "factory/yolov3_gluon_416_factory.hpp"
#include "common/runner/sync_detection_runner.hpp"

int main(int argc, char* argv[]) {
    auto factory = std::make_unique<dxapp::YOLOv3_gluon_416Factory>();
    dxapp::SyncDetectionRunner<dxapp::YOLOv3_gluon_416Factory> runner(std::move(factory));
    return runner.run(argc, argv);
}
