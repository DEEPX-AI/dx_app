...
/**
 * @brief Asynchronous post-processing example for YOLOv10 object detection
model.
 *
 ...
 */
void scale_coordinates(YOLOv10Result& detection, const std::vector<int>& pad_xy,
 ...
 */
cv::Mat draw_detections(const cv::Mat& frame, std::vector<YOLOv10Result>& detections,
    ...
    while (appQuit->load() == 0) {
        if (wait_queue->size() > 0) {
            ...
            // detections 멤버는 포인터이므로, 벡터를 동적할당하여 복사
            {
                ...
                display_args->detections = new std::vector<YOLOv10Result>(detections_vec);
...
int main(int argc, char* argv[]) {
    ...
    auto post_processor =
        YOLOv10PostProcess(input_width, input_height, 0.3f, 0.45f, ie.IsOrtConfigured());
