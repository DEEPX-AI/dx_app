#ifndef YOLOV7_POSTPROCESS_H
#define YOLOV7_POSTPROCESS_H
...
struct YOLOv7Result {
    ...
    YOLOv7Result() = default;

    // Parameterized constructor with move semantics for better performance
    YOLOv7Result(std::vector<float> box_val, const float conf, const int cls_id,
                 const std::string& cls_name)
        : box(std::move(box_val)), confidence(conf), class_id(cls_id), class_name(cls_name) {}

    // Legacy constructor for backward compatibility
    YOLOv7Result(const std::vector<float>& box_val, const float conf, const int cls_id,
                 const std::string& cls_name);
    ...
    YOLOv7Result(const YOLOv7Result& other) = default;
    YOLOv7Result& operator=(const YOLOv7Result& other) = default;
    YOLOv7Result(YOLOv7Result&& other) noexcept = default;
    YOLOv7Result& operator=(YOLOv7Result&& other) noexcept = default;
    ...
    float iou(const YOLOv7Result& other) const;
...
class YOLOv7PostProcess {
   private:
    ...
    std::vector<YOLOv7Result> decoding_cpu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOv7Result> decoding_npu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOv7Result> apply_nms(const std::vector<YOLOv7Result>& detections) const;
   ...
   public:
    ...
    YOLOv7PostProcess(const int input_w, const int input_h, const float obj_threshold,
     ...
     * @brief Destructor
     */
    virtual ~YOLOv7PostProcess() = default;
     ...
     */
    std::vector<YOLOv7Result> postprocess(const dxrt::TensorPtrs& outputs);
