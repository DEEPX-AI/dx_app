...
// YOLOv5PPUResult methods implementation
float YOLOv5PPUResult::iou(const YOLOv5PPUResult& other) const {
...
// Constructor
YOLOv5PPUPostProcess::YOLOv5PPUPostProcess(const int input_w, const int input_h,
...
std::vector<YOLOv5PPUResult> YOLOv5PPUPostProcess::decoding_ppu_outputs(
...
// Apply Non-Maximum Suppression
std::vector<YOLOv5PPUResult> YOLOv5PPUPostProcess::apply_nms(
    const std::vector<YOLOv5PPUResult>& detections) const {
    if (detections.empty()) {
        return {};
    }

    // Sort detections by confidence (descending)
    std::vector<YOLOv5PPUResult> sorted_detections = detections;
    std::sort(sorted_detections.begin(), sorted_detections.end(),
              [](const YOLOv5PPUResult& a, const YOLOv5PPUResult& b) {
                  return a.confidence > b.confidence;
              });

    std::vector<bool> suppressed(sorted_detections.size(), false);
    std::vector<YOLOv5PPUResult> result;
...
// Set thresholds
void YOLOv5PPUPostProcess::set_thresholds(float obj_threshold, float score_threshold,
                                           float nms_threshold) {
...
std::string YOLOv5PPUPostProcess::get_config_info() const {
    std::ostringstream oss;
    oss << "YOLOv5 PPU PostProcess Configuration:\n"
