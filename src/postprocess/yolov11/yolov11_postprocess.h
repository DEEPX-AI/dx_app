#ifndef YOLOV11_POSTPROCESS_H
#define YOLOV11_POSTPROCESS_H
...
struct YOLOv11Result {
    ...
    YOLOv11Result() = default;

    // Parameterized constructor with move semantics for better performance
    YOLOv11Result(std::vector<float> box_val, const float conf, const int cls_id,
                 const std::string& cls_name)
        : box(std::move(box_val)), confidence(conf), class_id(cls_id), class_name(cls_name) {}

    // Legacy constructor for backward compatibility
    YOLOv11Result(const std::vector<float>& box_val, const float conf, const int cls_id,
                 const std::string& cls_name);
    ...
    YOLOv11Result(const YOLOv11Result& other) = default;
    YOLOv11Result& operator=(const YOLOv11Result& other) = default;
    YOLOv11Result(YOLOv11Result&& other) noexcept = default;
    YOLOv11Result& operator=(YOLOv11Result&& other) noexcept = default;
    ...
    float iou(const YOLOv11Result& other) const;
...
class YOLOv11PostProcess {
   private:
    ...
    std::vector<YOLOv11Result> decoding_cpu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOv11Result> decoding_npu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOv11Result> apply_nms(const std::vector<YOLOv11Result>& detections) const;
   ...
   public:
    ...
    YOLOv11PostProcess(const int input_w, const int input_h, const float score_threshold,
     ...
     * @brief Destructor
     */
    virtual ~YOLOv11PostProcess() = default;
     ...
     */
    std::vector<YOLOv11Result> postprocess(const dxrt::TensorPtrs& outputs);
