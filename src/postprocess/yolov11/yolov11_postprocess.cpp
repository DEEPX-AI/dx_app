...
// YOLOv11Result methods implementation
float YOLOv11Result::iou(const YOLOv11Result& other) const {
...
bool YOLOv11Result::is_invalid(int image_width, int image_height) const {
    return box[0] < 0 || box[1] < 0 || box[2] > image_width || box[3] > image_height;
}

// Constructor
YOLOv11PostProcess::YOLOv11PostProcess(const int input_w, const int input_h,
    ...
    // Initialize model-specific parameters for YOLOv11
...
// Default constructor
YOLOv11PostProcess::YOLOv11PostProcess() {
    input_width_ = 640;
    input_height_ = 640;
...
std::vector<YOLOv11Result> YOLOv11PostProcess::postprocess(const dxrt::TensorPtrs& outputs) {
    ...
    std::vector<YOLOv11Result> detections;
...
dxrt::TensorPtrs YOLOv11PostProcess::align_tensors(const dxrt::TensorPtrs& outputs) const {
...
std::vector<YOLOv11Result> YOLOv11PostProcess::decoding_npu_outputs(
...
// Decode model outputs to detection results
std::vector<YOLOv11Result> YOLOv11PostProcess::decoding_cpu_outputs(
    const dxrt::TensorPtrs& outputs) const {
    std::vector<YOLOv11Result> detections;
    /**
     * @note Ultralytics models, which make yolov11/v8/v5-u/v7-u has same post
     ...
     *
     * yolov11.dxnn output shapes are follow.
     *     Outputs
...
// Apply Non-Maximum Suppression
std::vector<YOLOv11Result> YOLOv11PostProcess::apply_nms(
    const std::vector<YOLOv11Result>& detections) const {
    if (detections.empty()) {
        return {};
    }

    // Sort detections by confidence (descending)
    std::vector<YOLOv11Result> sorted_detections = detections;
    std::sort(
        sorted_detections.begin(), sorted_detections.end(),
        [](const YOLOv11Result& a, const YOLOv11Result& b) { return a.confidence > b.confidence; });

    std::vector<bool> suppressed(sorted_detections.size(), false);
    std::vector<YOLOv11Result> result;
...
// Set thresholds
void YOLOv11PostProcess::set_thresholds(float score_threshold, float nms_threshold) {
...
std::string YOLOv11PostProcess::get_config_info() const {
