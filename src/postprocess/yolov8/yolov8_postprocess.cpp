...
// YOLOv8Result methods implementation
float YOLOv8Result::iou(const YOLOv8Result& other) const {
...
bool YOLOv8Result::is_invalid(int image_width, int image_height) const {
    return box[0] < 0 || box[1] < 0 || box[2] > image_width || box[3] > image_height;
}

// Constructor
YOLOv8PostProcess::YOLOv8PostProcess(const int input_w, const int input_h,
    ...
    // Initialize model-specific parameters for YOLOv8n
...
std::vector<YOLOv8Result> YOLOv8PostProcess::postprocess(const dxrt::TensorPtrs& outputs) {
    auto aligned_outputs = align_tensors(outputs);
    if (aligned_outputs.empty()) {
        int i = 0;
        std::ostringstream msg;
        msg << "[DXAPP] [ERROR] YOLOv8PostProcess::postprocess - Aligned outputs are empty.\n"
            << "  Unexpected shape\n";
        for (auto& o : outputs) {
            msg << "    Output shape [" << i++ << "]: (";
    ...
    std::vector<YOLOv8Result> detections;
...
std::vector<YOLOv8Result> YOLOv8PostProcess::decoding_npu_outputs(
...
// Decode model outputs to detection results
std::vector<YOLOv8Result> YOLOv8PostProcess::decoding_cpu_outputs(
    const dxrt::TensorPtrs& outputs) const {
    std::vector<YOLOv8Result> detections;
    /**
...
// Apply Non-Maximum Suppression
std::vector<YOLOv8Result> YOLOv8PostProcess::apply_nms(
    const std::vector<YOLOv8Result>& detections) const {
    if (detections.empty()) {
        return {};
    }

    // Sort detections by confidence (descending)
    std::vector<YOLOv8Result> sorted_detections = detections;
    std::sort(
        sorted_detections.begin(), sorted_detections.end(),
        [](const YOLOv8Result& a, const YOLOv8Result& b) { return a.confidence > b.confidence; });
...
// Set thresholds
void YOLOv8PostProcess::set_thresholds(float score_threshold, float nms_threshold) {
...
std::string YOLOv8PostProcess::get_config_info() const {
...
dxrt::TensorPtrs YOLOv8PostProcess::align_tensors(const dxrt::TensorPtrs& outputs) const {
