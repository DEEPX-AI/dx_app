...
struct YOLOv8Result {
    ...
    YOLOv8Result(const std::vector<float>& box_val, const float conf, const int cls_id,
                 const std::string& cls_name);

    // Destructor
    ~YOLOv8Result() = default;
    YOLOv8Result(const YOLOv8Result&) = default;
    YOLOv8Result& operator=(const YOLOv8Result&) = default;
    YOLOv8Result(YOLOv8Result&&) noexcept = default;
    YOLOv8Result& operator=(YOLOv8Result&&) noexcept = default;
    ...
    float iou(const YOLOv8Result& other) const;
...
class YOLOv8PostProcess {
   private:
    ...
    std::vector<YOLOv8Result> decoding_cpu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOv8Result> decoding_npu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOv8Result> apply_nms(const std::vector<YOLOv8Result>& detections) const;
   ...
   public:
    ...
    YOLOv8PostProcess(const int input_w, const int input_h, const float score_threshold,
     ...
     */
    std::vector<YOLOv8Result> postprocess(const dxrt::TensorPtrs& outputs);
