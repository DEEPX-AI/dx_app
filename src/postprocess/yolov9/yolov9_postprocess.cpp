...
// YOLOv9Result methods implementation
float YOLOv9Result::iou(const YOLOv9Result& other) const {
...
bool YOLOv9Result::is_invalid(int image_width, int image_height) const {
    return box[0] < 0 || box[1] < 0 || box[2] > image_width || box[3] > image_height;
}

// Constructor
YOLOv9PostProcess::YOLOv9PostProcess(const int input_w, const int input_h,
...
std::vector<YOLOv9Result> YOLOv9PostProcess::postprocess(const dxrt::TensorPtrs& outputs) {
    ...
    if (aligned_outputs.empty()) {
        ...
        msg << "[DXAPP] [ERROR] YOLOv9PostProcess::postprocess - Aligned outputs are empty.\n"
            << "  Unexpected shape\n";
        for (auto& o : outputs) {
            msg << "    Output shape [" << i++ << "]: (";
    ...
    std::vector<YOLOv9Result> detections;
...
dxrt::TensorPtrs YOLOv9PostProcess::align_tensors(const dxrt::TensorPtrs& outputs) const {
...
std::vector<YOLOv9Result> YOLOv9PostProcess::decoding_npu_outputs(
...
// Decode model outputs to detection results
std::vector<YOLOv9Result> YOLOv9PostProcess::decoding_cpu_outputs(
    const dxrt::TensorPtrs& outputs) const {
    std::vector<YOLOv9Result> detections;
    /**
     * @note Ultralytics models, which make yolov9/v8/v5-u/v7-u has same post
     ...
     *
     * yolov9.dxnn output shapes are follow.
     *     Outputs
...
// Apply Non-Maximum Suppression
std::vector<YOLOv9Result> YOLOv9PostProcess::apply_nms(
    const std::vector<YOLOv9Result>& detections) const {
    if (detections.empty()) {
        return {};
    }

    // Sort detections by confidence (descending)
    std::vector<YOLOv9Result> sorted_detections = detections;
    std::sort(
        sorted_detections.begin(), sorted_detections.end(),
        [](const YOLOv9Result& a, const YOLOv9Result& b) { return a.confidence > b.confidence; });

    std::vector<bool> suppressed(sorted_detections.size(), false);
    std::vector<YOLOv9Result> result;
...
// Set thresholds
void YOLOv9PostProcess::set_thresholds(float score_threshold, float nms_threshold) {
...
std::string YOLOv9PostProcess::get_config_info() const {
    std::ostringstream oss;
    oss << "YOLOv9 PostProcess Configuration:\n"
