...
/**
 * @brief YOLOXs detection result structure
...
struct YOLOXResult {
    ...
    YOLOXResult(const std::vector<float>& box_val, const float conf, const int cls_id,
                const std::string& cls_name);
    ...
    float iou(const YOLOXResult& other) const;
...
/**
 * @brief YOLOXs post-processing class
...
class YOLOXPostProcess {
   private:
    ...
    std::vector<YOLOXResult> decoding_cpu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOXResult> decoding_npu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOXResult> apply_nms(const std::vector<YOLOXResult>& detections) const;
   ...
   public:
    ...
    YOLOXPostProcess(const int input_w, const int input_h, const float obj_threshold,
     ...
     */
    std::vector<YOLOXResult> postprocess(const dxrt::TensorPtrs& outputs);
