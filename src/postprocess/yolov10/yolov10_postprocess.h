#ifndef YOLOV10_POSTPROCESS_H
#define YOLOV10_POSTPROCESS_H
...
struct YOLOv10Result {
    ...
    YOLOv10Result(const std::vector<float>& box_val, const float conf, const int cls_id,
                 const std::string& cls_name);

    // Destructor
    ~YOLOv10Result() = default;
    YOLOv10Result(const YOLOv10Result&) = default;
    YOLOv10Result& operator=(const YOLOv10Result&) = default;
    YOLOv10Result(YOLOv10Result&&) noexcept = default;
    YOLOv10Result& operator=(YOLOv10Result&&) noexcept = default;
    ...
    float iou(const YOLOv10Result& other) const;
...
class YOLOv10PostProcess {
   private:
    ...
    std::vector<YOLOv10Result> decoding_cpu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOv10Result> decoding_npu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOv10Result> apply_nms(const std::vector<YOLOv10Result>& detections) const;
   ...
   public:
    ...
    YOLOv10PostProcess(const int input_w, const int input_h, const float score_threshold,
     ...
     */
    std::vector<YOLOv10Result> postprocess(const dxrt::TensorPtrs& outputs);
