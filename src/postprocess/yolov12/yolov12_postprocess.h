#ifndef YOLOV12_POSTPROCESS_H
#define YOLOV12_POSTPROCESS_H
...
struct YOLOv12Result {
    ...
    YOLOv12Result(const std::vector<float>& box_val, const float conf, const int cls_id,
                 const std::string& cls_name);

    // Destructor
    ~YOLOv12Result() = default;
    YOLOv12Result(const YOLOv12Result&) = default;
    YOLOv12Result& operator=(const YOLOv12Result&) = default;
    YOLOv12Result(YOLOv12Result&&) noexcept = default;
    YOLOv12Result& operator=(YOLOv12Result&&) noexcept = default;
    ...
    float iou(const YOLOv12Result& other) const;
...
class YOLOv12PostProcess {
   private:
    ...
    std::vector<YOLOv12Result> decoding_cpu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOv12Result> decoding_npu_outputs(const dxrt::TensorPtrs& outputs) const;
    std::vector<YOLOv12Result> apply_nms(const std::vector<YOLOv12Result>& detections) const;
   ...
   public:
    ...
    YOLOv12PostProcess(const int input_w, const int input_h, const float score_threshold,
     ...
     */
    std::vector<YOLOv12Result> postprocess(const dxrt::TensorPtrs& outputs);
