...
// YOLOv12Result methods implementation
float YOLOv12Result::iou(const YOLOv12Result& other) const {
...
bool YOLOv12Result::is_invalid(int image_width, int image_height) const {
    return box[0] < 0 || box[1] < 0 || box[2] > image_width || box[3] > image_height;
}

// Constructor
YOLOv12PostProcess::YOLOv12PostProcess(const int input_w, const int input_h,
    ...
    // Initialize model-specific parameters for YOLOv12
...
// Default constructor
YOLOv12PostProcess::YOLOv12PostProcess() {
    input_width_ = 640;
    input_height_ = 640;
...
std::vector<YOLOv12Result> YOLOv12PostProcess::postprocess(const dxrt::TensorPtrs& outputs) {
    ...
    if (aligned_outputs.empty()) {
        ...
        msg << "[DXAPP] [ERROR] YOLOv12PostProcess::postprocess - Aligned outputs are empty.\n"
            << "  Unexpected shape\n";
        for (auto& o : outputs) {
            msg << "    Output shape [" << i++ << "]: (";
    ...
    std::vector<YOLOv12Result> detections;
...
dxrt::TensorPtrs YOLOv12PostProcess::align_tensors(const dxrt::TensorPtrs& outputs) const {
...
std::vector<YOLOv12Result> YOLOv12PostProcess::decoding_npu_outputs(
...
// Decode model outputs to detection results
std::vector<YOLOv12Result> YOLOv12PostProcess::decoding_cpu_outputs(
    const dxrt::TensorPtrs& outputs) const {
    std::vector<YOLOv12Result> detections;
    /**
     * @note Ultralytics models, which make yolov12/v8/v5-u/v7-u has same post
     * processing methods (u: ultralytics)
     * (u: ultralytics) processing methods
     *      https://github.com/ultralytics/ultralytics/blob/main/examples/YOLOv8-ONNXRuntime-CPP/inference.cpp
     *
     * yolov12.dxnn output shapes are follow.
     *     Outputs
    ...
    for (size_t output_idx = 0; output_idx < outputs.size(); ++output_idx) {
        ...
        for (int i = 0; i < num_dets; ++i) {
            ...
            result.box.emplace_back(box_temp[0] - box_temp[2] / 2.0f);  // x1
            result.box.emplace_back(box_temp[1] - box_temp[3] / 2.0f);  // y1
            result.box.emplace_back(box_temp[0] + box_temp[2] / 2.0f);  // x2
            result.box.emplace_back(box_temp[1] + box_temp[3] / 2.0f);  // y2
...
// Apply Non-Maximum Suppression
std::vector<YOLOv12Result> YOLOv12PostProcess::apply_nms(
    const std::vector<YOLOv12Result>& detections) const {
    if (detections.empty()) {
        return {};
    }

    // Sort detections by confidence (descending)
    std::vector<YOLOv12Result> sorted_detections = detections;
    std::sort(
        sorted_detections.begin(), sorted_detections.end(),
        [](const YOLOv12Result& a, const YOLOv12Result& b) { return a.confidence > b.confidence; });

    std::vector<bool> suppressed(sorted_detections.size(), false);
    std::vector<YOLOv12Result> result;
...
// Set thresholds
void YOLOv12PostProcess::set_thresholds(float score_threshold, float nms_threshold) {
...
std::string YOLOv12PostProcess::get_config_info() const {
    std::ostringstream oss;
    oss << "YOLOv12 PostProcess Configuration:\n"
