...
extern YoloParam yolov5s_320, yolov5s_512, yolov5s_640, yolox_s_512, yolov7_640, yolov7_512;
...
int main(int argc, char *argv[])
{
    ...
    int cdma_fd = open("/dev/deepx_genesis_dma", O_RDWR);
    if (cdma_fd < 0) {
        cout << " open fail NPU Driver" <<endl;
		return -1;
	}
    ...
    if(asyncInference)
    {
        if(mode!=dxrt::InferenceMode::MODE_IO)
            mode = dxrt::InferenceMode::MODE_ASYNC;
    }

    string captionModel = dxrt::StringSplit(modelPath, "/").back() + ", 30fps";
    dxrt::InferenceOption option(mode, 0, 1, numBuf, false, true);
    ...
    if(!imgFile.empty())
    {
        ...
        if(mode==dxrt::InferenceMode::MODE_SYNC)
        {
            profiler.Start("main");
            vOutputs.emplace_back( ie.Run(resizedFrame.data) );
            profiler.End("main");
        }
        else if(mode==dxrt::InferenceMode::MODE_ASYNC)
        {
            int loop;
            ...
            {                
                ...
                profiler.End("main");
                LOG_VALUE(requests.back());
            }
            for(loop=0;loop<loops;loop++)
            {
                vOutputs.emplace_back(
                    ie.WaitForCompleted(requests[loop])
                );
            }
        }
                ...
                [&](vector<shared_ptr<dxrt::Tensor>> outputs, vector<shared_ptr<dxrt::Tensor>> inputs)
                {
                    ...
                    // cout << "      >> callback " << callBackCnt << endl; // debug
                        ...
                        bboxesQueue.push(
                            make_pair(
                                result, 
                                (uint64_t) outputs.front()->GetUserArgs()
                            )
                        );
                        lk.unlock();
                    profiler.End("post");
    ...
    if(ispInput)
    {
        ...
        {
            inputAddr.emplace_back(
                dxrt::MemIfPhyBase + ie.GetInput(i).front()->GetBase() + ie.GetInput(i).front()->GetOffset()
            );
        }
        uint64_t inputSize = ie.GetInputSize();
        int cdma_cnt = 0;
        ...
        do {
            ...
            profiler.Start("cdma");
            bufIdx = cnt%numBuf;
            npu_cdma_data->input_addr = inputAddr[bufIdx];
            npu_cdma_data->uyv_addr = pUyvData->PhysAddr;
            npu_cdma_data->input_size = inputSize;
            if (ioctl(cdma_fd, DMA_IOCTL_DATA, npu_cdma_data)){
                cdma_cnt++;
                if (cdma_cnt>10){
            ...
            profiler.End("cdma");
            profiler.Start("main");
            auto outputs = ie.Run(bufIdx);
            profiler.End("main");
#ifdef ISP_DEBUG_BY_OPENCV
            memcpy(frame.data, (UYV_DATA_S *)(pUyvData->pVirtAddr), ie.GetInputSize());
#endif
#else
            profiler.Start("main");
            ...
            cnt++;
            profiler.End("total");
            nputime[NPU_INFERENCE] = ie.GetNpuPerf(0);
        } while(loops<0?1:(cnt<loops));
