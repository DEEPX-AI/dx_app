...
- **C++ (`src/cpp_example/common/`)**: Base interfaces (`IFactory`, `IProcessor`, `IVisualizer`, `IInputSource`), 42 post-processors, 24 task-specific sync/async runner pairs, 11 visualizers, input source abstraction, config loader, utility
- **Python (`src/python_example/common/`)**: Base interfaces (`IFactory`, `IProcessor`, `IVisualizer`, `IInputSource`), 34 post-processors, generic `SyncRunner`/`AsyncRunner`, 9 visualizers, input source abstraction, `ModelConfig` loader, utility
...
- **`config/model_registry.json`**: centralized registry of 335 models with per-model metadata (task, postprocessor, input dimensions, thresholds)
