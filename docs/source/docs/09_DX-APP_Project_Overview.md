...
The refactored Python tree is organized by **task → model family → variant**, with a shared `common/` layer providing base interfaces, 34 post-processors, generic sync/async runners, 9 visualizers, and input abstraction — the same factory-based architecture as the C++ side. For structure and contributor-facing rules, refer to [DX-APP Python Usage Guide](05_DX-APP_Python_Example_Usage_Guide.md) and [DX-APP Example Source Structure](11_DX-APP_Example_Source_Structure.md).
