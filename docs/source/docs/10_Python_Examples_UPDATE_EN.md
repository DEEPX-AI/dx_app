...
[`dx_postprocess`](../../../src/bindings/python/dx_postprocess/) is a library that wraps the [post-processing classes](../../../src/postprocess) used in [C++ Examples](../../../src/cpp_example/) with `pybind11`, making them available in Python. It is used when performance optimization of Python-based post-processing is needed. Examples with `'_cpp_postprocess'` in their filenames use this library.
...
Install dependency packages like `Numpy` and `OpenCV` required to run the Python examples. Run the following command to install the packages listed in the [`requirements.txt`](../../../src/python_example/requirements.txt) file.
...
-   **Implementation**: 
[`dx_postprocess`](../../../src/bindings/python/dx_postprocess/) is a library that wraps the [post-processing classes](../../../src/postprocess) used in [C++ Examples](../../../src/cpp_example/) with `pybind11`, making them available in Python. `'_cpp_postprocess'` examples import this library and directly call the C++ implemented post-processing functions instead of the original Python logic.
...
### 5.1. Image Inference (`image_inference`)
...
-   **Performance Report**: After execution, it prints an **`Image Processing Summary`** to the console, showing the time taken for each processing step.
    ...
    <details>
    <summary>Example Image Processing Summary</summary>
...
### 5.2. Stream Inference (`stream_inference`)
...
-   **Performance Report**: When execution ends (video finishes, or `Esc`/`q` key is pressed), it prints a **`Performance Summary`** to the console, including **overall throughput (Overall FPS)** and other metrics.
    ...
    -   With the `--no-display` option, the `Display` item is excluded from the output.
    <details>
    <summary>Example Performance Summary (sync example)</summary>
    ...
    **Metric Explanation**:
    ...
    <details>
    <summary>Example Performance Summary (async example)</summary>
...
This section walks you through running the `object_detection` `yolov9` examples to directly observe key features and performance differences between variants.

### 6.1. Basic Image Inference (Sync)
...
## 7. Advanced: Bulk Performance Report Generation
