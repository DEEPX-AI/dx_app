...
| `dx-python-builder` | 4가지 변형으로 Python 추론 앱 빌드: `sync`, `async`, `sync_cpp_postprocess`, `async_cpp_postprocess` | — |
| `dx-cpp-builder` | `InferenceEngine` API를 사용한 C++ 추론 앱 빌드 | — |
| `dx-benchmark-builder` | 대상 하드웨어에서의 추론 성능 벤치마크 및 프로파일링 | — |
| `dx-model-manager` | `.dxnn` 컴파일 모델 다운로드, 등록, 관리 | — |
...
| 도구 | 사용 방법 |
|---|---|
| **Claude Code** | 프롬프트를 직접 입력. `CLAUDE.md`가 `dx-build-python-app` 스킬로 라우팅. variant, 작업 유형, 모델에 대해 2-3개 질문 후 `src/python_example/object_detection/yolo26n/`에 파일 생성 및 검증. |
