...
| `dx-receiving-code-review` | 기술적 엄밀성을 갖춘 코드 리뷰 피드백 수신 및 처리 |
| `dx-requesting-code-review` | 작업이 요구 사항을 충족하는지 검증하기 위한 코드 리뷰 요청 |
| `dx-skill-router` | 작업을 적절한 스킬로 자동 라우팅 |
| `dx-subagent-driven-development` | 독립 서브 에이전트를 활용한 구현 계획 실행 |
| `dx-systematic-debugging` | 체계적 디버깅 — 수정 제안 전 진단 수행 |
...
| `dx-writing-plans` | 코드 작성 전 구조화된 구현 계획 작성 |
...
| **OpenCode** | `AGENTS.md`, `opencode.json`, `.opencode/agents/`의 6개 에이전트, `.deepx/skills/`의 17개 스킬 | `@dx-app-builder` 또는 `/dx-build-python-app` |
...
| `/dx-dispatching-parallel-agents` | 독립 작업을 병렬 에이전트에 디스패치 |
| `/dx-executing-plans` | 리뷰 체크포인트가 포함된 구현 계획 실행 |
| `/dx-receiving-code-review` | 기술적 엄밀성을 갖춘 코드 리뷰 피드백 수신 |
| `/dx-requesting-code-review` | 작업 검증을 위한 코드 리뷰 요청 |
| `/dx-skill-router` | 적절한 스킬로 자동 라우팅 |
| `/dx-subagent-driven-development` | 독립 서브 에이전트를 활용한 계획 실행 |
| `/dx-systematic-debugging` | 체계적 디버깅 — 수정 전 진단 |
| `/dx-tdd` | 점진적 검증을 포함한 테스트 주도 개발 |
| `/dx-verify-completion` | 증거 기반 완료 검증 |
| `/dx-writing-plans` | 구조화된 구현 계획 작성 |
| `/dx-writing-skills` | 스킬 정의 생성 또는 편집 |
...
| `.cursor/rules/python-example.mdc` | — | — | — | ✅ | 자동 (glob) |
| `.cursor/rules/cpp-example.mdc` | — | — | — | ✅ | 자동 (glob) |
| `.cursor/rules/tests.mdc` | — | — | — | ✅ | 자동 (glob) |
