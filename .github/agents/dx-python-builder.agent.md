...
NEVER write demo scripts from scratch. Copy closest example from `src/python_example/<task>/` as skeleton, modify ONLY model-specific parts. Do NOT propose alternative implementation approaches (single image, wait(), register_callback() — these are fabricated).

## Never Reuse Previous Session Artifacts (MANDATORY)
NEVER check, list, browse, or reference files from previous sessions in `dx-agentic-dev/`. Always create a new session directory with a fresh timestamp.
