...
from framework.config import YOLOV8SEG_CONFIG
...
pytestmark = pytest.mark.yolov8seg


@pytest.mark.unit
class TestYOLOv8SegBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV8SEG_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV8SEG_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV8SEG_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv8SegGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV8SEG_CONFIG)
...
class TestYOLOv8SegIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV8SEG_CONFIG)
...
@pytest.mark.cli
class TestYOLOv8SegCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV8SEG_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv8SegE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV8SEG_CONFIG)
