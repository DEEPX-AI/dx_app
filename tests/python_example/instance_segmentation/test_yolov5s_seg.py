...
from framework.config import YOLOV5_SEG_CONFIG
...
pytestmark = pytest.mark.yolov5s_seg


@pytest.mark.unit
class TestYOLOv5sSegBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5_SEG_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5_SEG_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV5_SEG_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv5sSegGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5_SEG_CONFIG)
...
class TestYOLOv5sSegIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5_SEG_CONFIG)
...
@pytest.mark.cli
class TestYOLOv5sSegCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5_SEG_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv5sSegE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5_SEG_CONFIG)
