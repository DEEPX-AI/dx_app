...
from framework.config import YOLOV26SEG_CONFIG
...
pytestmark = pytest.mark.yolov26seg


@pytest.mark.unit
class TestYOLOv26SegBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV26SEG_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV26SEG_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV26SEG_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv26SegGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV26SEG_CONFIG)
...
class TestYOLOv26SegIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV26SEG_CONFIG)
...
@pytest.mark.cli
class TestYOLOv26SegCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV26SEG_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv26SegE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV26SEG_CONFIG)
