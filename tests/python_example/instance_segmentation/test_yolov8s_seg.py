...
from framework.config import YOLOV8S_SEG_CONFIG
...
pytestmark = pytest.mark.yolov8s_seg


@pytest.mark.unit
class TestYOLOv8sSegBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV8S_SEG_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV8S_SEG_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV8S_SEG_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv8sSegGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV8S_SEG_CONFIG)
...
class TestYOLOv8sSegIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV8S_SEG_CONFIG)
...
@pytest.mark.cli
class TestYOLOv8sSegCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV8S_SEG_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv8sSegE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV8S_SEG_CONFIG)
