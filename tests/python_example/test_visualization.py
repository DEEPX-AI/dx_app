...
def _build_vis_params():
    ...
    params = []
    for task, model_name, sync_script, async_script, model_path in raw:
        ...
        if sync_script is not None:
            params.append((task, model_name, sync_script, model_path, "sync", img_rel))
        if async_script is not None:
            params.append((task, model_name, async_script, model_path, "async", img_rel))
    return params
...
VIS_PARAMS = [
    pytest.param(task, name, script, model, mode, img, id=f"{name}_{mode}")
