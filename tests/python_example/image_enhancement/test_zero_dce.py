...
from framework.config import ZERO_DCE_CONFIG
...
pytestmark = pytest.mark.zero_dce


@pytest.mark.unit
class TestZeroDCEBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(ZERO_DCE_CONFIG)

    @pytest.mark.parametrize("script_name", ZERO_DCE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in ZERO_DCE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestZeroDCEGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(ZERO_DCE_CONFIG)
...
class TestZeroDCEIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(ZERO_DCE_CONFIG)
...
@pytest.mark.cli
class TestZeroDCECLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(ZERO_DCE_CONFIG)
...
@pytest.mark.e2e
class TestZeroDCEE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(ZERO_DCE_CONFIG)
