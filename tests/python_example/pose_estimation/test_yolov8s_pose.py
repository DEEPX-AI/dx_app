...
from framework.config import YOLOV8S_POSE_CONFIG
...
pytestmark = pytest.mark.yolov8s_pose


@pytest.mark.unit
class TestYOLOv8sPoseBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV8S_POSE_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV8S_POSE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV8S_POSE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv8sPoseGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV8S_POSE_CONFIG)
...
class TestYOLOv8sPoseIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV8S_POSE_CONFIG)
...
@pytest.mark.cli
class TestYOLOv8sPoseCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV8S_POSE_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv8sPoseE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV8S_POSE_CONFIG)
