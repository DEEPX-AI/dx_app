...
from framework.config import YOLOV8M_POSE_CONFIG
...
pytestmark = pytest.mark.yolov8m_pose


@pytest.mark.unit
class TestYOLOv8mPoseBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV8M_POSE_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV8M_POSE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV8M_POSE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv8mPoseGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV8M_POSE_CONFIG)
...
class TestYOLOv8mPoseIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV8M_POSE_CONFIG)
...
@pytest.mark.cli
class TestYOLOv8mPoseCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV8M_POSE_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv8mPoseE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV8M_POSE_CONFIG)
