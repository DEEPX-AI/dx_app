...
from framework.config import YOLOV5S_PPU_CONFIG
...
pytestmark = pytest.mark.yolov5s_ppu


@pytest.mark.unit
class TestYOLOv5sPPUBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5S_PPU_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5S_PPU_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV5S_PPU_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv5sPPUGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5S_PPU_CONFIG)
...
class TestYOLOv5sPPUIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5S_PPU_CONFIG)
...
@pytest.mark.cli
class TestYOLOv5sPPUCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5S_PPU_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv5sPPUE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5S_PPU_CONFIG)
