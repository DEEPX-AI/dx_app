...
from framework.config import YOLOV5POSE_PPU_CONFIG
...
pytestmark = pytest.mark.yolov5pose_ppu


@pytest.mark.unit
class TestYOLOv5PosePPUBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5POSE_PPU_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5POSE_PPU_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name",
        [s for s in YOLOV5POSE_PPU_CONFIG.variants if "cpp_postprocess" not in s],
    )
...
@pytest.mark.unit
class TestYOLOv5PosePPUGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5POSE_PPU_CONFIG)
...
class TestYOLOv5PosePPUIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5POSE_PPU_CONFIG)
...
@pytest.mark.cli
class TestYOLOv5PosePPUCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5POSE_PPU_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv5PosePPUE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5POSE_PPU_CONFIG)
