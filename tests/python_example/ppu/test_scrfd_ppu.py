...
from framework.config import SCRFD_PPU_CONFIG
...
pytestmark = pytest.mark.scrfd_ppu


@pytest.mark.unit
class TestSCRFDPPUBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(SCRFD_PPU_CONFIG)

    @pytest.mark.parametrize("script_name", SCRFD_PPU_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name",
        [s for s in SCRFD_PPU_CONFIG.variants if "cpp_postprocess" not in s],
    )
...
@pytest.mark.unit
class TestSCRFDPPUGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(SCRFD_PPU_CONFIG)
...
class TestSCRFDPPUIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(SCRFD_PPU_CONFIG)
...
@pytest.mark.cli
class TestSCRFDPPUCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(SCRFD_PPU_CONFIG)
...
@pytest.mark.e2e
class TestSCRFDPPUE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(SCRFD_PPU_CONFIG)
