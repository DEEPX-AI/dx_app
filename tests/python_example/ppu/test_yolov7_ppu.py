...
from framework.config import YOLOV7_PPU_CONFIG
...
pytestmark = pytest.mark.yolov7_ppu


@pytest.mark.unit
class TestYOLOv7PPUBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV7_PPU_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV7_PPU_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name",
        [s for s in YOLOV7_PPU_CONFIG.variants if "cpp_postprocess" not in s],
    )
...
@pytest.mark.unit
class TestYOLOv7PPUGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV7_PPU_CONFIG)
...
class TestYOLOv7PPUIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV7_PPU_CONFIG)
...
@pytest.mark.cli
class TestYOLOv7PPUCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV7_PPU_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv7PPUE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV7_PPU_CONFIG)
