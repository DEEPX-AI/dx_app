...
from framework.config import YOLOV5_PPU_CONFIG
...
pytestmark = pytest.mark.yolov5_ppu


@pytest.mark.unit
class TestYOLOv5PPUBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5_PPU_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5_PPU_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name",
        [s for s in YOLOV5_PPU_CONFIG.variants if "cpp_postprocess" not in s],
    )
...
@pytest.mark.unit
class TestYOLOv5PPUGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5_PPU_CONFIG)
...
class TestYOLOv5PPUIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5_PPU_CONFIG)
...
@pytest.mark.cli
class TestYOLOv5PPUCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5_PPU_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv5PPUE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5_PPU_CONFIG)
