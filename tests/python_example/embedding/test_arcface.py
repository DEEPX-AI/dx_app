...
from framework.config import ARCFACE_CONFIG
...
pytestmark = pytest.mark.arcface


@pytest.mark.unit
class TestArcFaceBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(ARCFACE_CONFIG)

    @pytest.mark.parametrize("script_name", ARCFACE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in ARCFACE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestArcFaceGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(ARCFACE_CONFIG)
...
class TestArcFaceIntegration:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(ARCFACE_CONFIG)
...
@pytest.mark.cli
class TestArcFaceCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(ARCFACE_CONFIG)
...
@pytest.mark.e2e
class TestArcFaceE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(ARCFACE_CONFIG)
