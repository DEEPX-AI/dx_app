...
from framework.config import OSNET_CONFIG
...
class TestOSNetBase:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(OSNET_CONFIG)

    @pytest.mark.parametrize("script_name", OSNET_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in OSNET_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestOSNetGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(OSNET_CONFIG)
...
class TestOSNetIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(OSNET_CONFIG)
...
class TestOSNetCLI:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(OSNET_CONFIG)
...
class TestOSNetE2E:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(OSNET_CONFIG)
