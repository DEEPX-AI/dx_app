...
from framework.config import RESNET50_CONFIG
...
pytestmark = pytest.mark.resnet50


@pytest.mark.unit
class TestResNet50Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(RESNET50_CONFIG)

    @pytest.mark.parametrize("script_name", RESNET50_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in RESNET50_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestResNet50Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(RESNET50_CONFIG)
...
class TestResNet50Integration:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(RESNET50_CONFIG)
...
@pytest.mark.cli
class TestResNet50CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(RESNET50_CONFIG)
...
class TestResNet50E2E:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(RESNET50_CONFIG)
