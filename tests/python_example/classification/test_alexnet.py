...
from framework.config import ALEXNET_CONFIG
...
pytestmark = pytest.mark.alexnet


@pytest.mark.unit
class TestAlexNetBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(ALEXNET_CONFIG)

    @pytest.mark.parametrize("script_name", ALEXNET_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in ALEXNET_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestAlexNetGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(ALEXNET_CONFIG)
...
class TestAlexNetIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(ALEXNET_CONFIG)
...
@pytest.mark.cli
class TestAlexNetCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(ALEXNET_CONFIG)
...
@pytest.mark.e2e
class TestAlexNetE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(ALEXNET_CONFIG)
