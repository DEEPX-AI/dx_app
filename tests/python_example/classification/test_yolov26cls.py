...
from framework.config import YOLOV26CLS_CONFIG
...
pytestmark = pytest.mark.yolov26cls


@pytest.mark.unit
class TestYOLOv26ClsBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV26CLS_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV26CLS_CONFIG.variants)
    def test_preprocess(self, script_name):
        self.framework.test_preprocess(script_name)

    @pytest.mark.parametrize(
        "script_name",
        [s for s in YOLOV26CLS_CONFIG.variants if "cpp_postprocess" not in s],
    )
...
@pytest.mark.unit
class TestYOLOv26ClsGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV26CLS_CONFIG)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV26CLS_CONFIG.variants if "_sync" in s]
    )
...
class TestYOLOv26ClsIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV26CLS_CONFIG)
...
@pytest.mark.cli
class TestYOLOv26ClsCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV26CLS_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv26ClsE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV26CLS_CONFIG)
