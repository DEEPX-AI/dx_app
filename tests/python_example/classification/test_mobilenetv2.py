...
from framework.config import MOBILENETV2_CONFIG
...
pytestmark = pytest.mark.mobilenetv2


@pytest.mark.unit
class TestMobileNetV2Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(MOBILENETV2_CONFIG)

    @pytest.mark.parametrize("script_name", MOBILENETV2_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in MOBILENETV2_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestMobileNetV2Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(MOBILENETV2_CONFIG)
...
class TestMobileNetV2Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(MOBILENETV2_CONFIG)
...
@pytest.mark.cli
class TestMobileNetV2CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(MOBILENETV2_CONFIG)
...
@pytest.mark.e2e
class TestMobileNetV2E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(MOBILENETV2_CONFIG)
