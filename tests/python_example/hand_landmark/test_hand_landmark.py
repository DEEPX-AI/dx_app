...
from framework.config import HAND_LANDMARK_CONFIG
...
pytestmark = pytest.mark.hand_landmark


@pytest.mark.unit
class TestHandLandmarkBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(HAND_LANDMARK_CONFIG)

    @pytest.mark.parametrize("script_name", HAND_LANDMARK_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in HAND_LANDMARK_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestHandLandmarkGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(HAND_LANDMARK_CONFIG)
...
class TestHandLandmarkIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(HAND_LANDMARK_CONFIG)
...
@pytest.mark.cli
class TestHandLandmarkCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(HAND_LANDMARK_CONFIG)
...
@pytest.mark.e2e
class TestHandLandmarkE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(HAND_LANDMARK_CONFIG)
