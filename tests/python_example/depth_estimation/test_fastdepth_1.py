...
from framework.config import FASTDEPTH_1_CONFIG
...
pytestmark = pytest.mark.fastdepth_1


@pytest.mark.unit
class TestFastDepth1Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(FASTDEPTH_1_CONFIG)

    @pytest.mark.parametrize("script_name", FASTDEPTH_1_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in FASTDEPTH_1_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestFastDepth1Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(FASTDEPTH_1_CONFIG)
...
class TestFastDepth1Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(FASTDEPTH_1_CONFIG)
...
@pytest.mark.cli
class TestFastDepth1CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(FASTDEPTH_1_CONFIG)
...
@pytest.mark.e2e
class TestFastDepth1E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(FASTDEPTH_1_CONFIG)
