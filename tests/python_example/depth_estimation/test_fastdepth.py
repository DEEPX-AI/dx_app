...
from framework.config import FASTDEPTH_CONFIG
...
pytestmark = pytest.mark.fastdepth


@pytest.mark.unit
class TestFastDepthBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(FASTDEPTH_CONFIG)

    @pytest.mark.parametrize("script_name", FASTDEPTH_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in FASTDEPTH_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestFastDepthGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(FASTDEPTH_CONFIG)
...
class TestFastDepthIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(FASTDEPTH_CONFIG)
...
@pytest.mark.cli
class TestFastDepthCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(FASTDEPTH_CONFIG)
...
@pytest.mark.e2e
class TestFastDepthE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(FASTDEPTH_CONFIG)
