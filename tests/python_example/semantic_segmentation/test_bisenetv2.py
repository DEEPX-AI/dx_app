...
from framework.config import BISENETV2_CONFIG
...
pytestmark = pytest.mark.bisenetv2


@pytest.mark.unit
class TestBiSeNetV2Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(BISENETV2_CONFIG)

    @pytest.mark.parametrize("script_name", BISENETV2_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in BISENETV2_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestBiSeNetV2Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(BISENETV2_CONFIG)
...
class TestBiSeNetV2Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(BISENETV2_CONFIG)
...
@pytest.mark.cli
class TestBiSeNetV2CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(BISENETV2_CONFIG)
...
@pytest.mark.e2e
class TestBiSeNetV2E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(BISENETV2_CONFIG)
