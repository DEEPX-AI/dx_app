...
from framework.config import SEGFORMER_CONFIG
...
pytestmark = pytest.mark.segformer


@pytest.mark.unit
class TestSegFormerBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(SEGFORMER_CONFIG)

    @pytest.mark.parametrize("script_name", SEGFORMER_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in SEGFORMER_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestSegFormerGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(SEGFORMER_CONFIG)
...
class TestSegFormerIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(SEGFORMER_CONFIG)
...
@pytest.mark.cli
class TestSegFormerCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(SEGFORMER_CONFIG)
...
@pytest.mark.e2e
class TestSegFormerE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(SEGFORMER_CONFIG)
