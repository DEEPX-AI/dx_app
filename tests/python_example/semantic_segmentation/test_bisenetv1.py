...
from framework.config import BISENETV1_CONFIG
...
pytestmark = pytest.mark.bisenetv1


@pytest.mark.unit
class TestBiSeNetV1Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(BISENETV1_CONFIG)

    @pytest.mark.parametrize("script_name", BISENETV1_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in BISENETV1_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestBiSeNetV1Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(BISENETV1_CONFIG)
...
class TestBiSeNetV1Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(BISENETV1_CONFIG)
...
@pytest.mark.cli
class TestBiSeNetV1CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(BISENETV1_CONFIG)
...
@pytest.mark.e2e
class TestBiSeNetV1E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(BISENETV1_CONFIG)
