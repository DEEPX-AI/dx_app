...
from framework.config import DEEPLABV3_CONFIG
...
pytestmark = pytest.mark.deeplabv3


@pytest.mark.unit
class TestDeepLabv3Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(DEEPLABV3_CONFIG)

    @pytest.mark.parametrize("script_name", DEEPLABV3_CONFIG.variants)
    def test_preprocess(self, script_name):
        self.framework.test_preprocess(script_name)

    @pytest.mark.parametrize(
        "script_name",
        [s for s in DEEPLABV3_CONFIG.variants if "cpp_postprocess" not in s],
    )
...
@pytest.mark.unit
class TestDeepLabv3Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(DEEPLABV3_CONFIG)
...
class TestDeepLabv3Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(DEEPLABV3_CONFIG)
...
@pytest.mark.cli
class TestDeepLabv3CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(DEEPLABV3_CONFIG)
...
@pytest.mark.e2e
class TestDeepLabv3E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(DEEPLABV3_CONFIG)
