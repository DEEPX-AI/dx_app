...
from framework.config import DNCNN_15_CONFIG
...
pytestmark = pytest.mark.dncnn


@pytest.mark.unit
class TestDnCNN15Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(DNCNN_15_CONFIG)

    @pytest.mark.parametrize("script_name", DNCNN_15_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in DNCNN_15_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestDnCNN15Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(DNCNN_15_CONFIG)
...
class TestDnCNN15Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(DNCNN_15_CONFIG)
...
@pytest.mark.cli
class TestDnCNN15CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(DNCNN_15_CONFIG)
...
@pytest.mark.e2e
class TestDnCNN15E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(DNCNN_15_CONFIG)
