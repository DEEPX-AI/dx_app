...
from framework.config import ESPCN_X4_CONFIG
...
pytestmark = pytest.mark.espcn


@pytest.mark.unit
class TestESPCNX4Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(ESPCN_X4_CONFIG)

    @pytest.mark.parametrize("script_name", ESPCN_X4_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in ESPCN_X4_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestESPCNX4Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(ESPCN_X4_CONFIG)
...
class TestESPCNX4Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(ESPCN_X4_CONFIG)
...
@pytest.mark.cli
class TestESPCNX4CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(ESPCN_X4_CONFIG)
...
@pytest.mark.e2e
class TestESPCNX4E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(ESPCN_X4_CONFIG)
