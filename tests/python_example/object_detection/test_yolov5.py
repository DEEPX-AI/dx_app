...
from framework.config import YOLOV5_CONFIG
...
pytestmark = pytest.mark.yolov5


@pytest.mark.unit
class TestYOLOv5Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV5_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv5Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5_CONFIG)
...
class TestYOLOv5Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5_CONFIG)
...
class TestYOLOv5CLI:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5_CONFIG)
...
class TestYOLOv5E2E:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5_CONFIG)
