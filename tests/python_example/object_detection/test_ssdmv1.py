...
from framework.config import SSDMV1_CONFIG
...
pytestmark = pytest.mark.ssdmv1


@pytest.mark.unit
class TestSSDMv1Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(SSDMV1_CONFIG)

    @pytest.mark.parametrize("script_name", SSDMV1_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in SSDMV1_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestSSDMv1Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(SSDMV1_CONFIG)
...
class TestSSDMv1Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(SSDMV1_CONFIG)
...
class TestSSDMv1CLI:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(SSDMV1_CONFIG)
...
class TestSSDMv1E2E:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(SSDMV1_CONFIG)
