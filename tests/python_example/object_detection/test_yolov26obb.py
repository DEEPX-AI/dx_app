...
from framework.config import YOLOV26OBB_CONFIG
...
pytestmark = pytest.mark.yolov26obb


@pytest.mark.unit
class TestYOLOv26ObbBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV26OBB_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV26OBB_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV26OBB_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv26ObbGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV26OBB_CONFIG)
...
class TestYOLOv26ObbIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV26OBB_CONFIG)
...
@pytest.mark.cli
class TestYOLOv26ObbCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV26OBB_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv26ObbE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV26OBB_CONFIG)
