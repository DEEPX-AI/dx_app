...
from framework.config import YOLOV5POSE_CONFIG
...
pytestmark = pytest.mark.yolov5pose


@pytest.mark.unit
class TestYOLOv5PoseBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5POSE_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5POSE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name",
        [s for s in YOLOV5POSE_CONFIG.variants if "cpp_postprocess" not in s],
    )
...
@pytest.mark.unit
class TestYOLOv5PoseGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5POSE_CONFIG)
...
class TestYOLOv5PoseIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5POSE_CONFIG)
...
@pytest.mark.cli
class TestYOLOv5PoseCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5POSE_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv5PoseE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5POSE_CONFIG)
