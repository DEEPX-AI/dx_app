...
from framework.config import YOLOV7_CONFIG
...
pytestmark = pytest.mark.yolov7


@pytest.mark.unit
class TestYOLOv7Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV7_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV7_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV7_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv7Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV7_CONFIG)
...
class TestYOLOv7Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV7_CONFIG)
...
class TestYOLOv7CLI:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV7_CONFIG)
...
class TestYOLOv7E2E:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV7_CONFIG)
