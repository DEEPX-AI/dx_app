...
from framework.config import YOLOV7TINY_CONFIG
...
pytestmark = pytest.mark.yolov7tiny


@pytest.mark.unit
class TestYOLOv7TinyBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV7TINY_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV7TINY_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV7TINY_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv7TinyGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV7TINY_CONFIG)
...
class TestYOLOv7TinyIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV7TINY_CONFIG)
...
@pytest.mark.cli
class TestYOLOv7TinyCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV7TINY_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv7TinyE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV7TINY_CONFIG)
