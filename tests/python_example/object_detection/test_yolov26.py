...
from framework.config import YOLOV26_CONFIG
...
pytestmark = pytest.mark.yolov26


@pytest.mark.unit
class TestYOLOv26Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV26_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV26_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV26_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv26Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV26_CONFIG)
...
class TestYOLOv26Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV26_CONFIG)
...
@pytest.mark.cli
class TestYOLOv26CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV26_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv26E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV26_CONFIG)
