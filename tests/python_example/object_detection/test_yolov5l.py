...
from framework.config import YOLOV5L_CONFIG
...
pytestmark = pytest.mark.yolov5l


@pytest.mark.unit
class TestYOLOv5lBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5L_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5L_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV5L_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv5lGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5L_CONFIG)
...
class TestYOLOv5lIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5L_CONFIG)
...
@pytest.mark.cli
class TestYOLOv5lCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5L_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv5lE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5L_CONFIG)
