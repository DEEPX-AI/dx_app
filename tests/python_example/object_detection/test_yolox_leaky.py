...
from framework.config import YOLOX_LEAKY_CONFIG
...
pytestmark = pytest.mark.yolox_leaky


@pytest.mark.unit
class TestYOLOXLeakyBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOX_LEAKY_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOX_LEAKY_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOX_LEAKY_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOXLeakyGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOX_LEAKY_CONFIG)
...
class TestYOLOXLeakyIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOX_LEAKY_CONFIG)
...
@pytest.mark.cli
class TestYOLOXLeakyCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOX_LEAKY_CONFIG)
...
@pytest.mark.e2e
class TestYOLOXLeakyE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOX_LEAKY_CONFIG)
