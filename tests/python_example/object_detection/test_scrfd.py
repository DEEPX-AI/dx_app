...
from framework.config import SCRFD_CONFIG
...
class TestSCRFDBase:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(SCRFD_CONFIG)

    @pytest.mark.parametrize("script_name", SCRFD_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in SCRFD_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestSCRFDGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(SCRFD_CONFIG)
...
class TestSCRFDIntegration:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(SCRFD_CONFIG)
...
class TestSCRFDCLI:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(SCRFD_CONFIG)
...
class TestSCRFDE2E:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(SCRFD_CONFIG)
