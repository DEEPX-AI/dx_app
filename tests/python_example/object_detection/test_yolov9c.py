...
from framework.config import YOLOV9C_CONFIG
...
pytestmark = pytest.mark.yolov9c


@pytest.mark.unit
class TestYOLOv9cBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV9C_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV9C_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV9C_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv9cGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV9C_CONFIG)
...
class TestYOLOv9cIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV9C_CONFIG)
...
@pytest.mark.cli
class TestYOLOv9cCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV9C_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv9cE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV9C_CONFIG)
