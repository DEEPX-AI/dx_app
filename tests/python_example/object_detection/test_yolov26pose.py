...
from framework.config import YOLOV26POSE_CONFIG
...
pytestmark = pytest.mark.yolov26pose


@pytest.mark.unit
class TestYOLOv26PoseBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV26POSE_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV26POSE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV26POSE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv26PoseGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV26POSE_CONFIG)
...
class TestYOLOv26PoseIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV26POSE_CONFIG)
...
@pytest.mark.cli
class TestYOLOv26PoseCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV26POSE_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv26PoseE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV26POSE_CONFIG)
