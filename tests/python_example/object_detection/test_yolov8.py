...
from framework.config import YOLOV8_CONFIG
...
pytestmark = pytest.mark.yolov8


@pytest.mark.unit
class TestYOLOv8Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV8_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV8_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV8_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv8Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV8_CONFIG)
...
class TestYOLOv8Integration:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV8_CONFIG)
...
class TestYOLOv8CLI:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV8_CONFIG)
...
class TestYOLOv8E2E:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV8_CONFIG)
