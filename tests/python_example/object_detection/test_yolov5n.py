...
from framework.config import YOLOV5N_CONFIG
...
pytestmark = pytest.mark.yolov5n


@pytest.mark.unit
class TestYOLOv5nBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5N_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5N_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV5N_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv5nGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5N_CONFIG)
...
class TestYOLOv5nIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5N_CONFIG)
...
@pytest.mark.cli
class TestYOLOv5nCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5N_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv5nE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5N_CONFIG)
