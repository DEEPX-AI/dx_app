...
from framework.config import YOLOV10_CONFIG
...
pytestmark = pytest.mark.yolov10


@pytest.mark.unit
class TestYOLOv10Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV10_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV10_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV10_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv10Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV10_CONFIG)
...
class TestYOLOv10Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV10_CONFIG)
...
@pytest.mark.cli
class TestYOLOv10CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV10_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv10E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV10_CONFIG)
