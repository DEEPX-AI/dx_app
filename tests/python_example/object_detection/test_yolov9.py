...
from framework.config import YOLOV9_CONFIG
...
pytestmark = pytest.mark.yolov9


@pytest.mark.unit
class TestYOLOv9Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV9_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV9_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV9_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv9Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV9_CONFIG)
...
class TestYOLOv9Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV9_CONFIG)
...
class TestYOLOv9CLI:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV9_CONFIG)
...
class TestYOLOv9E2E:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV9_CONFIG)
