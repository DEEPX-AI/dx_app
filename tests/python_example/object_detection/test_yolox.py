...
from framework.config import YOLOX_CONFIG
...
class TestYOLOXBase:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOX_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOX_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOX_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOXGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOX_CONFIG)
...
class TestYOLOXIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOX_CONFIG)
...
class TestYOLOXCLI:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOX_CONFIG)
...
class TestYOLOXE2E:
    ...
    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOX_CONFIG)
