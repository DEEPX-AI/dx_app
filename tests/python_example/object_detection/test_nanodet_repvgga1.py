...
from framework.config import NANODET_REPVGGA1_CONFIG
...
pytestmark = pytest.mark.nanodet


@pytest.mark.unit
class TestNanoDetRepVGGA1Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(NANODET_REPVGGA1_CONFIG)

    @pytest.mark.parametrize("script_name", NANODET_REPVGGA1_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in NANODET_REPVGGA1_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestNanoDetRepVGGA1Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(NANODET_REPVGGA1_CONFIG)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in NANODET_REPVGGA1_CONFIG.variants if "cpp_postprocess" in s]
    )
    def test_cpp_postprocess_no_method(self, script_name):
        self.framework.test_cpp_postprocess_no_method(script_name)

    @pytest.mark.parametrize(
        "script_name", [s for s in NANODET_REPVGGA1_CONFIG.variants if "_sync" in s]
    )
    def test_sync_has_image_inference(self, script_name):
        self.framework.test_sync_has_image_inference(script_name)

    @pytest.mark.parametrize(
        "script_name", [s for s in NANODET_REPVGGA1_CONFIG.variants if "_async" in s]
    )
...
class TestNanoDetRepVGGA1Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(NANODET_REPVGGA1_CONFIG)
...
@pytest.mark.cli
class TestNanoDetRepVGGA1CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(NANODET_REPVGGA1_CONFIG)
...
@pytest.mark.e2e
class TestNanoDetRepVGGA1E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(NANODET_REPVGGA1_CONFIG)
