...
from framework.config import YOLOV5M_CONFIG
...
pytestmark = pytest.mark.yolov5m


@pytest.mark.unit
class TestYOLOv5mBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5M_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5M_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV5M_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv5mGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5M_CONFIG)
...
class TestYOLOv5mIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5M_CONFIG)
...
@pytest.mark.cli
class TestYOLOv5mCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5M_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv5mE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5M_CONFIG)
