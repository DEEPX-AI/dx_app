...
from framework.config import YOLOV5FACE_CONFIG
...
pytestmark = pytest.mark.yolov5face


@pytest.mark.unit
class TestYOLOv5FaceBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5FACE_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5FACE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name",
        [s for s in YOLOV5FACE_CONFIG.variants if "cpp_postprocess" not in s],
    )
...
@pytest.mark.unit
class TestYOLOv5FaceGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5FACE_CONFIG)
...
class TestYOLOv5FaceIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5FACE_CONFIG)
...
@pytest.mark.cli
class TestYOLOv5FaceCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5FACE_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv5FaceE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5FACE_CONFIG)
