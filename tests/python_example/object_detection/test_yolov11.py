...
from framework.config import YOLOV11_CONFIG
...
pytestmark = pytest.mark.yolov11


@pytest.mark.unit
class TestYOLOv11Base:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV11_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV11_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV11_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv11Groups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV11_CONFIG)
...
class TestYOLOv11Integration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV11_CONFIG)
...
@pytest.mark.cli
class TestYOLOv11CLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV11_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv11E2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV11_CONFIG)
