...
from framework.config import NANODET_REPVGG_CONFIG
...
@pytest.mark.unit
class TestNanoDetRepVGGBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(NANODET_REPVGG_CONFIG)

    @pytest.mark.parametrize("script_name", NANODET_REPVGG_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in NANODET_REPVGG_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestNanoDetRepVGGGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(NANODET_REPVGG_CONFIG)
...
class TestNanoDetRepVGGIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(NANODET_REPVGG_CONFIG)
...
@pytest.mark.cli
class TestNanoDetRepVGGCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(NANODET_REPVGG_CONFIG)
...
@pytest.mark.e2e
class TestNanoDetRepVGGE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(NANODET_REPVGG_CONFIG)
