...
from framework.config import DAMOYOLO_CONFIG
...
pytestmark = pytest.mark.damoyolo


@pytest.mark.unit
class TestDAMOYOLOBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(DAMOYOLO_CONFIG)

    @pytest.mark.parametrize("script_name", DAMOYOLO_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in DAMOYOLO_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestDAMOYOLOGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(DAMOYOLO_CONFIG)
...
class TestDAMOYOLOIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(DAMOYOLO_CONFIG)
...
@pytest.mark.cli
class TestDAMOYOLOCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(DAMOYOLO_CONFIG)
...
@pytest.mark.e2e
class TestDAMOYOLOE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(DAMOYOLO_CONFIG)
