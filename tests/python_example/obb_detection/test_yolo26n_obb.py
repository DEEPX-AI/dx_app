...
from framework.config import YOLO26N_OBB_CONFIG
...
pytestmark = pytest.mark.yolo26n_obb


@pytest.mark.unit
class TestYOLO26nOBBBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLO26N_OBB_CONFIG)

    @pytest.mark.parametrize("script_name", YOLO26N_OBB_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLO26N_OBB_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLO26nOBBGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLO26N_OBB_CONFIG)
...
class TestYOLO26nOBBIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLO26N_OBB_CONFIG)
...
@pytest.mark.cli
class TestYOLO26nOBBCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLO26N_OBB_CONFIG)
...
@pytest.mark.e2e
class TestYOLO26nOBBE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLO26N_OBB_CONFIG)
