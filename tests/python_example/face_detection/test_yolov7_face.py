...
from framework.config import YOLOV7_FACE_CONFIG
...
pytestmark = pytest.mark.yolov7_face


@pytest.mark.unit
class TestYOLOv7FaceBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV7_FACE_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV7_FACE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV7_FACE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv7FaceGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV7_FACE_CONFIG)
...
class TestYOLOv7FaceIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV7_FACE_CONFIG)
...
@pytest.mark.cli
class TestYOLOv7FaceCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV7_FACE_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv7FaceE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV7_FACE_CONFIG)
