...
from framework.config import YOLOV7_W6_FACE_CONFIG
...
pytestmark = pytest.mark.yolov7_w6_face


@pytest.mark.unit
class TestYOLOv7W6FaceBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV7_W6_FACE_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV7_W6_FACE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV7_W6_FACE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv7W6FaceGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV7_W6_FACE_CONFIG)
...
class TestYOLOv7W6FaceIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV7_W6_FACE_CONFIG)
...
@pytest.mark.cli
class TestYOLOv7W6FaceCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV7_W6_FACE_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv7W6FaceE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV7_W6_FACE_CONFIG)
