...
from framework.config import YOLOV7S_FACE_CONFIG
...
pytestmark = pytest.mark.yolov7s_face


@pytest.mark.unit
class TestYOLOv7sFaceBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV7S_FACE_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV7S_FACE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV7S_FACE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv7sFaceGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV7S_FACE_CONFIG)
...
class TestYOLOv7sFaceIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV7S_FACE_CONFIG)
...
@pytest.mark.cli
class TestYOLOv7sFaceCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV7S_FACE_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv7sFaceE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV7S_FACE_CONFIG)
