...
from framework.config import RETINAFACE_CONFIG
...
pytestmark = pytest.mark.retinaface


@pytest.mark.unit
class TestRetinaFaceBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(RETINAFACE_CONFIG)

    @pytest.mark.parametrize("script_name", RETINAFACE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in RETINAFACE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestRetinaFaceGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(RETINAFACE_CONFIG)
...
class TestRetinaFaceIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(RETINAFACE_CONFIG)
...
@pytest.mark.cli
class TestRetinaFaceCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(RETINAFACE_CONFIG)
...
@pytest.mark.e2e
class TestRetinaFaceE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(RETINAFACE_CONFIG)
