...
from framework.config import YOLOV7_W6_TTA_FACE_CONFIG
...
pytestmark = pytest.mark.yolov7_w6_tta_face


@pytest.mark.unit
class TestYOLOv7W6TTAFaceBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV7_W6_TTA_FACE_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV7_W6_TTA_FACE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV7_W6_TTA_FACE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv7W6TTAFaceGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV7_W6_TTA_FACE_CONFIG)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV7_W6_TTA_FACE_CONFIG.variants if "cpp_postprocess" in s]
    )
    def test_cpp_postprocess_no_method(self, script_name):
        self.framework.test_cpp_postprocess_no_method(script_name)

    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV7_W6_TTA_FACE_CONFIG.variants if "_sync" in s]
    )
    def test_sync_has_image_inference(self, script_name):
        self.framework.test_sync_has_image_inference(script_name)

    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV7_W6_TTA_FACE_CONFIG.variants if "_async" in s]
    )
...
class TestYOLOv7W6TTAFaceIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV7_W6_TTA_FACE_CONFIG)
...
@pytest.mark.cli
class TestYOLOv7W6TTAFaceCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV7_W6_TTA_FACE_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv7W6TTAFaceE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV7_W6_TTA_FACE_CONFIG)
