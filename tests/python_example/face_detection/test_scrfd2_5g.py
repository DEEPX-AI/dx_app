...
from framework.config import SCRFD2_5G_CONFIG
...
pytestmark = pytest.mark.scrfd2_5g


@pytest.mark.unit
class TestSCRFD2_5GBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(SCRFD2_5G_CONFIG)

    @pytest.mark.parametrize("script_name", SCRFD2_5G_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in SCRFD2_5G_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestSCRFD2_5GGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(SCRFD2_5G_CONFIG)
...
class TestSCRFD2_5GIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(SCRFD2_5G_CONFIG)
...
@pytest.mark.cli
class TestSCRFD2_5GCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(SCRFD2_5G_CONFIG)
...
@pytest.mark.e2e
class TestSCRFD2_5GE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(SCRFD2_5G_CONFIG)
