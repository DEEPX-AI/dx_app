...
from framework.config import SCRFD10G_CONFIG
...
pytestmark = pytest.mark.scrfd10g


@pytest.mark.unit
class TestSCRFD10GBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(SCRFD10G_CONFIG)

    @pytest.mark.parametrize("script_name", SCRFD10G_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in SCRFD10G_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestSCRFD10GGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(SCRFD10G_CONFIG)
...
class TestSCRFD10GIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(SCRFD10G_CONFIG)
...
@pytest.mark.cli
class TestSCRFD10GCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(SCRFD10G_CONFIG)
...
@pytest.mark.e2e
class TestSCRFD10GE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(SCRFD10G_CONFIG)
