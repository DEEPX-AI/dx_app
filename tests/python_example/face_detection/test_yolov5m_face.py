...
from framework.config import YOLOV5M_FACE_CONFIG
...
pytestmark = pytest.mark.yolov5m_face


@pytest.mark.unit
class TestYOLOv5mFaceBase:

    @classmethod
    def setup_class(cls):
        cls.framework = BaseTestFramework(YOLOV5M_FACE_CONFIG)

    @pytest.mark.parametrize("script_name", YOLOV5M_FACE_CONFIG.variants)
    ...
    @pytest.mark.parametrize(
        "script_name", [s for s in YOLOV5M_FACE_CONFIG.variants if "cpp_postprocess" not in s]
    )
...
@pytest.mark.unit
class TestYOLOv5mFaceGroups:

    @classmethod
    def setup_class(cls):
        cls.framework = GroupsTestFramework(YOLOV5M_FACE_CONFIG)
...
class TestYOLOv5mFaceIntegration:

    @classmethod
    def setup_class(cls):
        cls.framework = IntegrationTestFramework(YOLOV5M_FACE_CONFIG)
...
@pytest.mark.cli
class TestYOLOv5mFaceCLI:

    @classmethod
    def setup_class(cls):
        cls.framework = CLITestFramework(YOLOV5M_FACE_CONFIG)
...
@pytest.mark.e2e
class TestYOLOv5mFaceE2E:

    @classmethod
    def setup_class(cls):
        cls.framework = E2ETestFramework(YOLOV5M_FACE_CONFIG)
