...
with a fresh timestamp. Even if a previous session built the same model,
always start from scratch. Do NOT run `ls dx-agentic-dev/` or check for
existing files from past runs.
...
|---|---|---|
| 0 | Run `sanity_check.sh --dx_rt` to verify dx-runtime (judge by TEXT output, not exit code — see below) | FAIL → `--all --exclude-app --exclude-stream --skip-uninstall --venv-reuse` → re-run sanity_check → **STOP if still failing (unconditional — user cannot override).** If NPU hardware init failure → guide user to cold boot/reboot, then STOP |
