...
task_to_postprocessors() {
    case "$1" in
        ...
        depth_estimation)        echo "fastdepth" ;;
        image_denoising)         echo "dncnn" ;;
        super_resolution)        echo "espcn" ;;
        image_enhancement)       echo "zero_dce" ;;
        embedding)               echo "arcface clip_image clip_text" ;;
        ppu)                     echo "yolov5_ppu yolov7_ppu yolov5pose_ppu" ;;
        hand_landmark)           echo "hand_landmark" ;;
    esac
}
...
do_run() {
    ...
    if [[ ${#extra_args[@]} -eq 0 ]]; then
        ...
        local _model_names
        _model_names=($(_get_all_model_names))
        _read_complete "$(echo -e "${CYAN}▸${NC} Filter (Enter = all): ")" filter "${_model_names[@]}"
