...
void YoloPostProcess::SetConfig(py::dict config)
{
    if (config.contains("model"))
    {
        py::dict model_dict = config["model"].cast<py::dict>();
    ...
    for (int cls = 0; cls < _param._numClasses; cls++)
    {
        if (ScoreIndices[cls].size() > 0)
        {
            ...
            for (int i = 0; i < numCandidates; i++)
            {
                ...
                Results.push_back((Boxes[4 * ScoreIndices[cls][i].second] - _pad.first) / _ratio.first);
                Results.push_back((Boxes[4 * ScoreIndices[cls][i].second + 1] - _pad.second) / _ratio.second);
                Results.push_back((Boxes[4 * ScoreIndices[cls][i].second + 2] - _pad.first) / _ratio.first);
                Results.push_back((Boxes[4 * ScoreIndices[cls][i].second + 3] - _pad.second) / _ratio.second);  
                ...
                if (Keypoints.size() > 0)
                {
                    ...
                    {
                        Results.push_back((Keypoints[3 * _param._numKeypoints * ScoreIndices[cls][i].second + 3 * k] - _pad.first) / _ratio.first);
                        Results.push_back((Keypoints[3 * _param._numKeypoints * ScoreIndices[cls][i].second + 3 * k + 1] - _pad.second) / _ratio.second);
...
py::array_t<float> YoloPostProcess::Run(py::list ie_output,
                                        ...
                                        std::pair<float, float> pad = {0.0f, 0.0f})
{
    _ratio = ratio;
    _pad = pad;
